-- Fixes pApiCrearArticuloInventario's duplicate-co_art error message, which
-- previously leaked the CHAR(30) padding (trailing spaces) of @sCoArt into
-- the user-facing RAISERROR text, e.g.:
--   "El código de artículo 0000172                        ya existe"
-- RTRIM the value before raising. A local variable is used (rather than
-- inlining RTRIM(@sCoArt) directly in the RAISERROR argument list) because
-- this SQL Server build has previously shown itself fussy about non-trivial
-- expressions as RAISERROR arguments (see GETDATE() precedent elsewhere in
...
BEGIN
    ...
    BEGIN TRY
        ...
        BEGIN
            DECLARE @sCoArtTrimmed VARCHAR(30) = RTRIM(@sCoArt);
            RAISERROR('El código de artículo %s ya existe', 16, 1, @sCoArtTrimmed);
        END
