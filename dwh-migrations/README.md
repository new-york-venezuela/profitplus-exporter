...
`Fact_Sales.UnitCost`/`COGSAmount`/`GrossProfitAmount`/`CostSourceFlag` are wired up, sourced from `stg.RecipeLine` (`0031_stg_recipe_cost_snapshot.sql`) — a mirror of each active recipe's ingredient list, repopulated by `scripts/dwh-recipe-cost-load.ts` from the exporter app's own Recipes module (SQLite `recipes`/`recipe_lines`) — combined with `dwh.fn_IngredientCostAsOf` (`0032_fact_sales_recipe_cost.sql`), a point-in-time FIFO function that reconstructs each raw material's remaining layers as of a given date from `Ncake_a.dbo.saCostoHistoricoEntrada`/`saCostoHistoricoSalida`. `dwh.Load_Fact_Sales` calls this once per distinct (product, sale date) pair, so cost reflects the sale's own date, not "the latest" cost; `dwh.Backfill_Fact_Sales_RecipeCost` (same migration, called by the same loader) keeps already-loaded historical rows in sync the same way, since the watermark-gated MERGE never revisits unchanged ERP rows on its own. `stg.RecipeCostSnapshot` still exists and is still populated by the same loader, but only backs the Analytics "latest cost per product" column and a future cost-over-time chart — it is not the source for `Fact_Sales`'s cost columns.
