...
def _ensure_dw_database(pyodbc, server: str, port: str, db: str, user: str, pwd: str,
                         encrypt: str, trust: str) -> None:
    master = pyodbc.connect(
        _conn_str(server, port, "master", user, pwd, encrypt, trust), autocommit=True
    )
    try:
...
def _get_connections():
    ...
    _ensure_dw_database(pyodbc, dw_server, dw_port, dw_db, user, pwd, encrypt, trust)
...
SELECT
    ...
    ISNULL(f.doc_num, '')                               AS Numero_Factura
...
INSERT INTO Fact_Ventas
    (Fecha_Key, Cliente_Key, Articulo_Key, Vendedor_Key,
     Cantidad, Monto_Neto_BS, Tasa_Documento, Numero_Factura)
VALUES (?, ?, ?, ?, ?, ?, ?, ?);
"""

# A/R snapshot: CXC neta por cliente (documento neto de notas de crédito sin aplicar)
AR_SNAPSHOT_SRC = """
SELECT
    co_cli,
...
# Inventory snapshot: stock per warehouse, valued at average purchase cost, indexed to latest USD rate.
# NOTE: saStockAlmacen has a `tipo` column with observed values ACT/LLE (meaning undocumented in the
# knowledge base — possibly "disponible" vs "en tránsito/reservado"). This query SUMS across tipo,
# i.e. Stock_Actual = total physical stock regardless of tipo. If LLE means "not yet available for
# sale", this snapshot overstates sellable stock — confirm the business meaning of `tipo` before
# using Snapshot_Inventario for reorder-point decisions.
INV_SNAPSHOT_SRC = """
SELECT
    s.co_art,
