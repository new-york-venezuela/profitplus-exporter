...
| `nac` | bit(1,0) | NULL | — | — |
| `co_cta_ingr_egr` | char(20) | NULL | b'C\xc3\xb3digo de la Cuenta de Ingreso/Egreso asociado al documento' | FK → `saCuentaIngEgr.co_cta_ingr_egr` |

## Triggers Relacionados
- `TrigEstado_saDevolucionProveedor`

## Foreign Keys (explícitas)
- `FK_saDevolucionProveedor_saCondicionPago`: `co_cond` → `saCondicionPago.co_cond`
- `FK_saDevolucionProveedor_saCuentaIngEgr`: `co_cta_ingr_egr` → `saCuentaIngEgr.co_cta_ingr_egr`
- `FK_saDevolucionProveedor_saDocumentoCompra`: `co_tipo_doc` → `saDocumentoCompra.co_tipo_doc`
- `FK_saDevolucionProveedor_saDocumentoCompra`: `nro_doc` → `saDocumentoCompra.nro_doc`
- `FK_saDevolucionProveedor_saMoneda`: `co_mone` → `saMoneda.co_mone`
- `FK_saDevolucionProveedor_saMovimientoBanco`: `mov_num_b` → `saMovimientoBanco.mov_num`
- `FK_saDevolucionProveedor_saMovimientoCaja`: `mov_num_c` → `saMovimientoCaja.mov_num`
- `FK_saDevolucionProveedor_saProveedor`: `co_prov` → `saProveedor.co_prov`
- `FK_saDevolucionProveedor_saTax`: `salestax` → `saTax.tax_id`
