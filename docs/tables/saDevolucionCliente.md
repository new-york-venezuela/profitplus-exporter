...
| `impfisfac` | varchar(15) | NULL | b'Numero de factura fiscal' | — |
...
| `imp_nro_z` | varchar(20) | NULL | — | — |

## Triggers Relacionados
- `TrigEstado_saDevolucionCliente`

## Foreign Keys (explícitas)
- `FK_saDevolucionCliente_saCliente`: `co_cli` → `saCliente.co_cli`
- `FK_saDevolucionCliente_saCondicionPago`: `co_cond` → `saCondicionPago.co_cond`
- `FK_saDevolucionCliente_saDocumentoVenta`: `co_tipo_doc` → `saDocumentoVenta.co_tipo_doc`
- `FK_saDevolucionCliente_saDocumentoVenta`: `nro_doc` → `saDocumentoVenta.nro_doc`
- `FK_saDevolucionCliente_saMoneda`: `co_mone` → `saMoneda.co_mone`
- `FK_saDevolucionCliente_saMovimientoBanco`: `mov_num_b` → `saMovimientoBanco.mov_num`
- `FK_saDevolucionCliente_saMovimientoCaja`: `mov_num_c` → `saMovimientoCaja.mov_num`
- `FK_saDevolucionCliente_saTax`: `salestax` → `saTax.tax_id`
