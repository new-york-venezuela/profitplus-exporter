...
| `Rowguid_Lote` | uniqueidentifier | NOT NULL | b'Rowguid del lote de entrada al que esta consumiendo' | FK → `saLoteEntrada.rowguid` |
...
- `FK_saLoteSalida_saAlmacen`: `co_alma` → `saAlmacen.co_alma`
- `FK_saLoteSalida_saArticulo`: `co_art` → `saArticulo.co_art`
- `FK_saLoteSalida_saLoteEntrada`: `Rowguid_Lote` → `saLoteEntrada.rowguid`
