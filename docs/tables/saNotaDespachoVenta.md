...
| `co_conductor` | char(6) | NULL | b'Codigo de transporte' | FK → `saTransporte.co_tran` |
| `imp_nro_z` | char(15) | NULL | — | — |

## Triggers Relacionados
- `TrigEstado_saNotaDespachoVenta`

## Foreign Keys (explícitas)
- `FK_saNotaDespachoVenta_saConductor`: `co_conductor` → `saTransporte.co_tran`
- `FK_saNotaDespachoVenta_saCuentaIngEgr`: `co_cta_ingr_egr` → `saCuentaIngEgr.co_cta_ingr_egr`
- `FK_saNotaDespachoVenta_saCliente`: `co_cli` → `saCliente.co_cli`
- `FK_saNotaDespachoVenta_saCondicionPago`: `co_cond` → `saCondicionPago.co_cond`
- `FK_saNotaDespachoVenta_saMoneda`: `co_mone` → `saMoneda.co_mone`
- `FK_saNotaDespachoVenta_saTax`: `salestax` → `saTax.tax_id`
- `FK_saNotaDespachoVenta_saTransporte`: `co_tran` → `saTransporte.co_tran`
- `FK_saNotaDespachoVenta_saVendedor`: `co_ven` → `saVendedor.co_ven`
