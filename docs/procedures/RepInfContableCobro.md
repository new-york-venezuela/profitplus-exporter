...
-- Description:	<Información Contable Cobro>
-- =============================================
CREATE PROCEDURE [dbo].[RepInfContableCobro]
...
BEGIN
  ...
  Select A.cob_num, COB.fecha, COB.co_cli, (select cli_des from saCliente where co_cli = COB.co_cli) as cli_des, COB.co_ven, 
  COB.feccom, COB.numcom, A.num_carp, A.Descripcion, case when ltrim(A.monto) = '' then 0 else cast(A.monto as decimal(18,5)) end as monto, 
 ...
 Select	A.cob_num, A.feccom, '01' as num_carp,
		...
		C.N.value('(Porcentaje)[1]', 'varchar(12)'
```
