# SP: pSeleccionarContabilizacionDevolucionProveedorRenglon
**Tipo**: Seleccionar
**Módulo**: Inventario
...
## Código (excerpt)
```sql
CREATE PROCEDURE [pSeleccionarContabilizacionDevolucionProveedorRenglon]
    (
...
AS 
    BEGIN
		...
		SELECT     FV.doc_num AS Co_Doc_Padre, FVR.reng_num AS Co_Doc, CASE WHEN FV.fec_emis > FV.fec_reg THEN FV.fec_emis ELSE FV.fec_reg END as fec_Emis, MO.relacion AS mone_relacion, CL.co_prov AS Co_Auxiliar, CL.prov_des AS Descrip_Auxiliar, 
                      FVR.co_sucu_in AS Co_Sucu_Cont, FVR.co_art, FVR.des_art, FVR.co_alma, FVR.total_art, FVR.stotal_art, FVR.co_uni, FVR.sco_uni, '' AS co_precio, '' AS prec_vta, 
                      ...
                      FVR.lote_asignado, FVR.dis_cen AS dis_cen_saDevolucionProveedorRenglon, FVR.co_us_in, FVR.co_sucu_in, FVR.fe_us_in, FVR.co_us_mo, FVR.co_sucu_mo, 
                      FVR.fe_us_mo, FVR.revisado, FVR.trasnfe, FVR.rowguid, FVR.pendiente2, '' AS prec_vta_om, AR.dis_cen AS dis_cen_saArticulo, FV.co_mone, 
                      CL.prov_des AS des_proveedor, CL.co_prov AS Co_proveedor, FV.tasa, CA.dis_cen AS dis_cen_saCatArticulo, LI.dis_cen AS dis_cen_saLineaArticulo,isnull(rx.sin_der_cre_fis,0) as sin_der_cre_fis,rx.credito_fiscal, FV.nro_fact AS nro_fact
		FROM         dbo.saDevolucionProveedorReng AS FVR INNER JOIN
                      dbo.saDevolucionProveedor AS FV ON FVR.doc_num = FV.doc_num INNER JOIN
                      ...
                      dbo.saLineaArticulo AS LI ON AR.co_lin = LI.co_lin left outer join
```
