...
/*=============================================
 ...
 Create date:   <14-04-10>
 Description:	<Compuesto Generado por Fecha>
 =============================================*/
CREATE PROCEDURE [RepCompuestoGenxFecha]
...
--VALOR POR DEFECTO
        ...
        SELECT
            ARTC.co_art AS Art_Compuesto, ARTC.descrip AS Descri_comp, ARTE.art_des AS Artículo_des,
            ARTE.modelo AS Modelo_articulo, ARTC.co_uni, A.*
        FROM
        ...
        WHERE
            ...
            AND ( ( @sdFec_Emis_d IS NULL
                    OR DATEADD(dd, 0, A.fecha) >= @sdFec_Emis_d
                  )
                  AND ( @sdFec_Emis_h IS NULL
                        OR DATEDIFF(dd, 0, A.fecha) <= @sdFec_Emis_h
                      )
                )
