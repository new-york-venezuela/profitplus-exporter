# SP: pValidarPendienteDevolucionClienteReng
**Tipo**: Validar
**Módulo**: Ventas
...
## Código (excerpt)
```sql
CREATE PROCEDURE [pValidarPendienteDevolucionClienteReng]
    (
...
AS 
    BEGIN	
        ...
        WHILE @@FETCH_STATUS = 0 
            BEGIN
                SET @PistaMensaje = 'La devolucion de cliente nro. "' + RTRIM(@pDoc_Num) + '" renglon "'
                ...
                IF ( @bCorregir = 1 ) 
                    BEGIN
                        ...
                        EXEC [pInsertarPista] @sUsuario_Id = 'VALCON', @dtFecha = @HoraCorrida, @sCo_Sucu = NULL,
                            @sTablaOri = 'saDevolucionClienteReng', @rowguidOri = @Id, @sTipo_Op = N'M',
                            @sMaquina = NULL, @sCampos = @PistaMensaje
                    END


                SET @HoraCorrida = GETDATE()
                EXEC [pInsertarPista] @sUsuario_Id = 'VALCON', @dtFecha = @HoraCorrida, @sCo_Sucu = NULL,
                    @sTablaOri = 'saDevolucionClienteReng', @rowguidOri = @IdProcess, @sTipo_Op = N'M', @sMaquina = NULL,
