...
-- =============================================
CREATE PROCEDURE [dbo].[RepPagosxProveedor]
    ...
    BEGIN
        ...
        FROM
            ( 
                ...
                P.cob_num, 
                           CASE 
                                  WHEN (TP.tipo_mov = 'CR' AND DC.co_tipo_doc = 'ADEL' AND PD.mont_cob > 0.00)
                    OR ( TP.tipo_mov = 'CR' AND DC.co_tipo_doc <> 'ADEL') THEN PD.mont_cob ELSE 0.00 
                           END 
                           * case 
                                  when p.anulado=1 then 0 else 1 
                           end AS abono, 
                           CASE 
                                  WHEN ( TP.tipo_mov = 'CR' AND DC.co_tipo_doc = 'ADEL' AND PD.mont_cob = 0) THEN DC.total_neto
                                  ELSE ( 
                                         CASE
```
