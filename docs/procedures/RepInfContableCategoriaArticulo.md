# SP: RepInfContableCategoriaArticulo
**Tipo**: Reporte
**Módulo**: Inventario
...
-- Description:	<Información Contable de Categoria>
-- =============================================
CREATE PROCEDURE [dbo].[RepInfContableCategoriaArticulo] 
...
BEGIN
	SET NOCOUNT ON;
   Select A.co_cat, A.num_carp, A.Descripcion, case when ltrim(A.monto) = '' then 0 else cast(A.monto as decimal(18,5)) end as monto,
   ...
   Select	A.co_cat, '01' as num_carp,
...
from	saCatArticulo A
		outer apply A.dis_cen.nodes('InformacionContable/Carpeta01/DistribucionCentros/CentroCosto') as C(N)
Where 
		((@sCo_Cat_d IS NULL OR A.co_cat >= @sCo_Cat_d)		AND ( @sCo_Cat_h IS NULL OR A.co_cat <= @sCo_Cat_h))
...
select	A.co_cat, '02' as num_carp,
		...
		A.dis_cen.query( '/InformacionContable/Carpeta02/CuentaContable').value('/','varchar(20)') as
```
