# SP: pSeleccionarContabilizacionGiroVenta
**Tipo**: Seleccionar
**Módulo**: Tesorería
...
## Código (excerpt)
```sql
CREATE PROCEDURE [pSeleccionarContabilizacionGiroVenta]
    (
...
AS 
	...
	--   SET @tiene_reng = (SELECT (CASE WHEN COUNT(co_tipo_doc) > 0 THEN 1 ELSE 0 END) FROM saDocumentoCompraReng WHE
```
