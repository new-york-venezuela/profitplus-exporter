# SP: pInsertarRenglonesFacturaCompraExt
**Tipo**: Insertar
**Módulo**: Compras
...
## Código (excerpt)
```sql
CREATE PROC [dbo].[pInsertarRenglonesFacturaCompraExt]
...
BEGIN 
	...
	IF EXISTS(SELECT rowguid_reng FROM saFacturaCompraRengExt WHERE rowguid_reng= @gRowGuid_Reng)
	BEGIN
		DELETE FROM saFacturaCompraRengExt WHERE rowguid_reng = @gRowGuid_Reng
	END

	DECLARE @bSin_Der_Cre_Fis BIT

	--IF (@sCredito_fiscal = '0')
		SET @bSin_Der_Cre_Fis = NULL
	--ELSE
	--	SET @bSin_Der_Cre_Fis = 0

	/*IF @bSin_Der_Cre_Fis = 1
	BEGIN*/
		INSERT INTO saFacturaCompraRengExt (
		...
		/*SELECT
			*
		FROM
			@TableTimestamp*/ -- Esta consulta causa conflicto al intentar obtener el rowguid de los renglones de factura de compra
	END
```
