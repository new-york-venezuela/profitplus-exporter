# SP: pSeleccionarContabilizacionNotaCreditoCompra
**Tipo**: Seleccionar
**Módulo**: Compras
...
## Código (excerpt)
```sql
CREATE PROCEDURE [dbo].[pSeleccionarContabilizacionNotaCreditoCompra]
    (
...
AS 
	...
	--   SET @tiene_reng = (SELECT (CASE WHEN COUNT(co_tipo_doc) > 0 THEN 1 ELSE 0 END) FROM saD
```
