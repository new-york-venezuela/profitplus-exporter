...
-- Description:	<Reporte de Factura de Compras por Número>
-- =============================================
CREATE PROCEDURE [RepCompraxNum] 
...
-------------------------------
        SELECT
            ...
            FC.doc_num, FC.nro_fact, FC.descrip, FC.co_prov, FC.co_mone, FC.co_cond, FC.fec_emis, FC.fec_venc,
            FC.fec_reg, FC.anulado, FC.status, FC.n_control, FC.tasa, FC.porc_desc_glob, FC.monto_desc_glob,
            FC.monto_reca, FC.total_bruto, 
			FC.monto_imp / ( CASE WHEN @cCo_Moneda IS NULL THEN 1
                                   ELSE FC.tasa
                              END ) AS monto_imp, 
			FC.otros1, FC.otros2, FC.otros3,
            FC.total_neto / ( CASE WHEN @cCo_Moneda IS NULL THEN 1
                                   ELSE FC.tasa
                              END ) AS total_neto, FC.saldo / ( CASE WHEN @cCo_Moneda IS NULL THEN 1
                                                                     ELSE FC.tasa
                                                                END ) AS saldo, FC.porc_reca, FC.monto_imp2,
            FC.monto_imp3, FC.dir_ent, FC.comentario, FC.dis_cen, FC.feccom, FC.numcom, FC.impresa, FC.salestax,
            FC.campo1, FC.campo2, FC.campo3, FC.campo4, FC.campo5, FC.campo6, FC.campo
```
