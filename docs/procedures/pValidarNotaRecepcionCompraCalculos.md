# SP: pValidarNotaRecepcionCompraCalculos
**Tipo**: Validar
**Módulo**: Compras
...
## Código (excerpt)
```sql
CREATE PROCEDURE [dbo].[pValidarNotaRecepcionCompraCalculos]
    (
...
AS 
	...
	-- Total Bruto (Versus renglones)
            ...
            SELECT
                ...
                END AS cobrosasociados, CASE WHEN ( E.numcom IS NULL
                                        ...
                                        END AS contabilizada, CASE WHEN ( E.status <> 0 ) THEN 1
                                                              ...
                                                              END AS procesada
            FROM
                saNotaRecepcionCompra E
                INNER JOIN saNotaRecepcionCompraReng R ON e.doc_num = R.doc_num
            GROUP BY
        ...
        WHILE @@FETCH_STATUS = 0 
            BEGIN
                SET @PistaMensaje = 'La nota de recepción nro. "' + RTRIM(@pDoc_Num)
