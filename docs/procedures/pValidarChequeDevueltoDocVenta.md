# SP: pValidarChequeDevueltoDocVenta
**Tipo**: Validar
**Módulo**: Tesorería
...
-- Description:	<pValidarChequeDevueltoDocVenta>
-- =============================================
CREATE PROCEDURE [dbo].[pValidarChequeDevueltoDocVenta]
	(
...
AS
	BEGIN
		DECLARE @valPendienteResult TABLE ( Motivo VARCHAR(1024))		
		...
		4ta parte
		*/
		--Valida que el código de ducumento de la tabla documento de venta sea 'CHEQ'
			SELECT DISTINCT
				'El documento de venta "' +  RTRIM(DV.nro_doc) +  '" no posee cheque devuelto asociado. *NC.' 
				as motivo, NULL as ID1, NULL as ID2, NULL as ValCli, 0 as MontBru, 0 as IVA, 
				0 as MontNeto, 0 as tasa, 0 as valor, NULL as Caract20
			FROM
				saDocumentoVenta DV
				LEFT JOIN saChequeDevueltoVenta CDV ON CDV.nro_doc = DV.nro_doc
			WHERE CDV.co_cheq_dev IS NULL and DV.co_tipo_doc='CHEQ'
