...
-- Description:	<Facturas de Compra por cuenta de ingreso/egreso>
-- =============================================
CREATE PROCEDURE [dbo].[RepFacturaCompraCtaIngrEgr]
	...
	BEGIN
		...
		SELECT
				...
				, CASE
					...
					END AS saldo
				, FC.anulado
		FROM   saFacturaCompra AS FC
					INNER JOIN saProveedor AS PR ON FC.co_prov = PR.co_prov
					LEFT JOIN saCuentaIngEgr AS CI ON FC.co_cta_ingr_egr = CI.co_cta_ingr_egr
		WHERE ((@sNumero_d IS NULL OR FC.doc_num >= @sNumero_d)
					AND (@sNumero_h IS NULL OR FC.doc_num <= @sNumero_h))
					AND ((@dFecha_d IS NULL OR dbo.FechaSimple(FC.fec_emis) >= @dFecha_d)
					AND (@dFecha_h IS NULL OR dbo.FechaSimple(FC.fec_emis) <= @dFecha_h))
					AND ((@sCo_pro_d IS NULL OR PR.co_prov >= @
```
