...
-- Description:	<Documentos de Compra por Fecha>
-- =============================================
CREATE PROCEDURE [dbo].[RepDocumentoCXPxFecha]
    ...
    @sPago_Aut CHAR(2) = NULL ,
    @iPago_Aut_Filtro INT = NULL ,
    @sImprSubTotal CHAR(6) = NULL ,
...
AS 
    BEGIN
        ...
        IF ( @sImprSubTotal IS NULL ) 
            SET @sImprSubTotal = 'SI'
        ...
        SELECT
            @sImprSubTotal AS SubTotales, DC.nro_doc, DC.co_tipo_doc, DC.nro_fact, 
			DC.co_prov, 
			...
			END AS tasa, 
			DC.co_mone as co_mone_doc, DC.tasa as tasa_doc, 
			MN.relacion as Rel_Inv,
			@sCo_Moneda as Mon_Fil, @sCo_Moneda_Rep as Mon_Rep,
			P.prov_des, TP.descrip, TP.tipo_mov
        FROM
            saDoc
```
