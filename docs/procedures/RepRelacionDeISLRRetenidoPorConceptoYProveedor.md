# SP: RepRelacionDeISLRRetenidoPorConceptoYProveedor
**Tipo**: Reporte
**Módulo**: Tesorería
...
-- Description:   <Relación de ISLR Retenido por Concepto y Proveedor>
-- =============================================
CREATE PROCEDURE [RepRelacionDeISLRRetenidoPorConceptoYProveedor]
    ...
    BEGIN
        ...
        IF @dFecha_h IS NOT NULL 
			...
			BEGIN 
				...
				SELECT
					rif, co_prov, prov_des, monto_obj, monto_reten, co_islr, porc_retn, sustraendo,
					( monto_reten - sustraendo ) AS imp_reten, islr_des, 'tipo' = @tipo
				FROM
					( SELECT
						P.rif, P.co_prov, P.prov_des, SUM(PR.monto_obj) AS monto_obj, (SUM(PR.monto_reten) + SUM(PR.sustraendo)) AS monto_reten,
						PR.co_islr, /*SUM(*/ PR.porc_retn/*) AS porc_retn*/, SUM(PR.sustraendo) sustraendo, C.islr_des
					  FROM
						saProveedor P
