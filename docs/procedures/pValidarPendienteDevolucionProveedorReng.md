# SP: pValidarPendienteDevolucionProveedorReng
**Tipo**: Validar
**Módulo**: Compras
...
## Código (excerpt)
```sql
CREATE PROCEDURE [pValidarPendienteDevolucionProveedorReng]
    (
...
AS 
    BEGIN	
        ...
        WHILE @@FETCH_STATUS = 0 
            BEGIN
                SET @PistaMensaje = 'La devolucion de proveedor nro. "' + RTRIM(@pDoc_Num) + '" renglon "'
                ...
                IF ( @bCorregir = 1 ) 
                    BEGIN
                        ...
                        EXEC [pInsertarPista] @sUsuario_Id = 'VALCON', @dtFecha = @HoraCorrida, @sCo_Sucu = NULL,
                            @sTablaOri = 'saDevolucionProveedorReng', @rowguidOri = @Id, @sTipo_Op = N'M',
                ...
                SET @HoraCorrida = GETDATE()
                EXEC [pInsertarPista] @sUsuario_Id = 'VALCON', @dtFecha = @HoraCorrida, @sCo_Sucu = NULL,
                    @sTablaOri = 'saDevolucionProveedorReng', @rowguidOri = @IdProcess, @sTipo_Op = N'M',
                    @sMaquina = NULL, @sCampos = @PistaMensaje
