# SP: RepMovimientoBancoNumeroTurno
**Tipo**: Reporte
**Módulo**: Tesorería
...
```sql
/**************************************************************************************************************
NOMBRE: RepMovimientoBancoNumeroTurno
DESCRIPCION: Reporte de Movimientos de Banco por Turno
CREADO POR: SOFTECH SISTEMAS
***************************************************************************************************************/
CREATE PROCEDURE [dbo].[RepMovimientoBancoNumeroTurno]
...
AS 
 ...
 (SELECT
		 ...
		 tu.co_turno, tu.des_turno, pv.cod_caja,
		 ...
		 case when de.anulado = 1 then 'Anulado' else '' end as anulado, Cub. co_mone, Mon.mone_des
  ...
  FROM   pvturnoexe as pv
	 ...
	 INNER JOIN saCuentaBancaria	  as Cub on Cub.cod_cta = De.cod_cta  -- Jortiz se agregan saCuentaBancaria y saMoneda para poder monstrar tipo de moneda y descripcion de moneda en el reporte
	 INNER JOIN saMoneda              as Mon on Mon.co_mone = Cub.co_mone -- Jortiz
     ...
     WHERE   
        ...
        AND (@dFecha_d IS NULL     OR dbo.FechaSimple(De.fecha) >= dbo.FechaSimple(@dFecha_d))
        AND (@dFecha_h I
```
