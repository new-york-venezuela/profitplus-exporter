# SP: pExportarActualizarRegistrosConfigArticulo
**Tipo**: Procedimiento
**Módulo**: Inventario
...
```sql
/**************************************************************************
*NOMBRE			:	pExportarActualizarRegistrosConfigArticulo
*DESCRIPCIÓN	:	Actualiza la configuracion del proceso Articulo
*AUTOR			:	SOFTECH SISTEMAS
***************************************************************************/
CREATE PROCEDURE [dbo].[pExportarActualizarRegistrosConfigArticulo]
    (
      @sCo_Config		CHAR(6) ,
...
AS 
    BEGIN
		...
		SET @sSqlValidador = N'	SELECT @tsValidadorOUT = validador FROM  [' + @sempresa + '].[dbo].[saConfigArticulo] WHERE co_config = @sCo_Config'  
		...
		SET @sSql = N'EXEC [' + @sempresa + '].[dbo].[pActualizarConfigArticulo]
																		@sCo_Config,
																		@sCo_ConfigOri,
																		@sDes_Config,
																		@sCo_Usuario,
