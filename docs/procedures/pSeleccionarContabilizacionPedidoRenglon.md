# SP: pSeleccionarContabilizacionPedidoRenglon
**Tipo**: Seleccionar
**Módulo**: Inventario
...
-- Description:	<pSeleccionarContabilizacionPedidoRenglon>
-- =============================================
CREATE PROCEDURE [dbo].[pSeleccionarContabilizacionPedidoRenglon]
    (
...
AS 
    BEGIN
		SELECT     FV.doc_num AS Co_Doc_Padre, FVR.reng_num AS Co_Doc, FV.fec_emis, MO.relacion AS mone_relacion, CL.co_cli AS Co_Auxiliar, CL.cli_des AS Descrip_Auxiliar, 
                      ...
                      FVR.lote_asignado, FVR.dis_cen AS dis_cen_saPedidoVentaRenglon, FVR.co_us_in, FVR.co_sucu_in, FVR.fe_us_in, FVR.co_us_mo, FVR.co_sucu_mo, FVR.fe_us_mo, 
                      FVR.revisado, FVR.trasnfe, FVR.rowguid, FVR.pendiente2, FVR.prec_vta_om, AR.dis_cen AS dis_cen_saArticulo, FV.co_mone, CL.cli_des AS des_cliente, 
                      CL.co_cli AS Co_cliente, FV.tasa, ca.dis_cen AS dis_cen_saCatArticulo, li.dis_cen AS dis_cen_saLineaArticulo
		FROM         dbo.saArticulo AS AR INNER JOIN
                      ...
                      dbo.saPedidoVentaReng AS FVR INNER JOIN
                      dbo.saPedidoVenta AS FV ON FVR.doc_num = FV.doc_num INNER JOIN
                      dbo.saMoneda AS MO ON FV.co_mone = MO.co_mone LEFT OUTER JOIN
                      dbo.saCliente AS CL ON FV.co_cli = CL.co_cli ON AR.co_art = FVR.co_art        
        WHERE
