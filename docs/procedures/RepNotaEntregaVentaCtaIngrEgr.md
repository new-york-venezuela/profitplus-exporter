# SP: RepNotaEntregaVentaCtaIngrEgr
**Tipo**: Reporte
**Módulo**: Clientes
...
-- =============================================
CREATE PROCEDURE [dbo].[RepNotaEntregaVentaCtaIngrEgr]
...
AS
	BEGIN
		...
		FROM   saNotaEntregaVenta AS NEV
					INNER JOIN saCliente AS CL ON NEV.co_cli = CL.co_cli
					LEFT JOIN saCuentaIngEgr AS CI ON NEV.co_cta_ingr_egr = CI.co_cta_ingr_egr
		WHERE ((@cNumero_d IS NULL OR NEV.doc_num >= @cNumero_d) 
					AND (@cNumero_h IS NULL OR NEV.doc_num <= @cNumero_h)) 
					AND ((@dFecha_d IS NULL OR dbo.FechaSimple(NEV.fec_emis) >= @dFecha_d) 
					AND (@dFecha_h IS NULL OR dbo.FechaSimple(NEV.fec_emis) <= @dFecha_h))
					AND ((@cCo_cli_d IS NU
```
