...
-- Description:	<Reporte de Ordenes de Compras por Artículos>
-- =============================================
CREATE PROCEDURE [dbo].[RepOrdenCompraxArt]
...
-------------------------------
        ...
        SELECT
			...
			FC.total_bruto, 
				 ...
				 --FC.total_neto / ( CASE WHEN @cCo_Moneda IS NULL THEN 1.00000000
     --                              ELSE FC.tasa
     --                         END ) AS total_neto, FC.saldo / ( CASE WHEN @cCo_Moneda IS NULL THEN 1.00000000
     --                                                                ELSE FC.tasa
     --                                                           END ) AS saldo, 

			CONVERT(DECIMAL(18,2),FC.total_neto / ( CASE WHEN @cCo_Moneda IS NULL THEN 1
														ELSE FC.tasa 
														END ) ) AS total_neto,

			CONVERT(DECIMAL(18,2),FC.saldo / ( CASE WHEN @cCo_Moneda IS NULL THEN 1
												ELSE FC.tasa
```
