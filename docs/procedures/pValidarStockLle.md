...
## Código (excerpt)
```sql
CREATE PROCEDURE [pValidarStockLle]
...
AS 
    BEGIN
        ...
        FOR
            ...
            FROM
                ( SELECT
                    co_art, co_alma, SUM(total_art) AS total_art, 'LLE' AS tipo_stock
                  FROM
                    ( SELECT
                        *
                      FROM
                ...
                FULL OUTER JOIN saStockAlmacen ON TABLA_CONSOLIDADA_STOCK_LLE.co_alma = saStockAlmacen.co_alma
                                                  ...
                                                  AND ( TABLA_CONSOLIDADA_STOCK_LLE.tipo_stock = 'LLE'
                                                        OR saStockAlmacen.tipo = 'LLE'
                                                      )
            WHERE
                ( TABLA_CONSOLIDADA_STOCK_LLE.tipo_stock = 'LLE'
                  OR saStockAlmacen.tipo = 'LLE'
                )
