# SP: pSeleccionarRenglonesNotaRecepcionCompra
**Tipo**: Seleccionar
**Módulo**: Inventario
...
***************************************************************************************************************/
CREATE PROCEDURE [pSeleccionarRenglonesNotaRecepcionCompra] ( @sDoc_Num CHAR(20) )
AS 
    BEGIN 
        SELECT
        ...
        FROM
            saNotaRecepcionCompraReng f
            INNER JOIN saArticulo a ON ( f.co_art = a.co_art )
            INNER JOIN saNotaRecepcionCompra E ON E.doc_num = f.doc_num
        WHERE
            f.doc_num = @sDoc_Num
        ORDER BY
            reng_num ASC
