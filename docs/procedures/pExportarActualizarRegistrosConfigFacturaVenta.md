# SP: pExportarActualizarRegistrosConfigFacturaVenta
**Tipo**: Procedimiento
**Módulo**: Ventas
...
```sql
/**************************************************************************
*NOMBRE			:	pExportarActualizarRegistrosConfigFacturaVenta
*DESCRIPCIÓN	:	Actualiza la configuracion del proceso Factura Venta
*AUTOR			:	SOFTECH SISTEMAS
***************************************************************************/
CREATE PROCEDURE [dbo].[pExportarActualizarRegistrosConfigFacturaVenta]
    (
      @sCo_Config		CHAR(6) ,
...
AS 
    BEGIN
		...
		SET @sSqlValidador = N'	SELECT @tsValidadorOUT = validador FROM  [' + @sempresa + '].[dbo].[saConfigFacturaVenta] WHERE co_config = @sCo_Config'  
		...
		SET @sSql = N'EXEC [' + @sempresa + '].[dbo].[pActualizarConfigFacturaVenta]
																		@sCo_Config,
																		@sCo_ConfigOri,
																		@sDes_Config,
																		@sCo_Usuario,
