# SP: pObtenerListadoDatosChequeDevueltoVenta
**Tipo**: Obtener
**Módulo**: Tesorería
...
```sql
/**************************************************************************************************************
NOMBRE: [pObtenerListadoDatosChequeDevueltoVenta]
DESCRIPCION: Obtiene la lista de documentos segun los parametros de entrada
CREADO POR: SOFTECH SISTEMAS
***************************************************************************************************************/
CREATE PROCEDURE [dbo].[pObtenerListadoDatosChequeDevueltoVenta]
    (
	  ...
	  @bAplica_TipoDevolucion		BIT					= NULL,
	  @sTipoDevolucion				CHAR(1)				= NULL,
    ...
    BEGIN	
		...
		ELSE 
          ...
          SET @SqlString = N'USE ' + CONVERT(NVARCHAR(100), @sDataBase_Name) + ' SELECT TOP(500) *    
					   FROM ' + CONVERT(NVARCHAR(100), 'saChequeDevueltoVenta') + ' WHERE '
		...
		IF (@sValor <> '')
			BEGIN
				...
				IF ( @iOpcion = 4 )--Fonetica
					BEGIN
						SET @SqlString = @SqlString + N' dbo.SoundexBusqueda(' + CONVERT(NVARC
```
