# SP: pInsertarConsecutivoEmpresa
**Tipo**: Insertar
**Módulo**: General
...
CREATE PROCEDURE [pInsertarConsecutivoEmpresa]
    (
      @sCo_Emp CHAR(20) ,
      @sCo_Us_In CHAR(6)
    )
        ...
        WHILE @@FETCH_STATUS = 0 
            BEGIN
                IF NOT ( EXISTS ( SELECT
                                  ...
                                  WHERE
                                    ( @sCo_Emp IS NULL
                                      AND saConsecutivo.co_emp IS NULL
                                      AND saConsecutivo.co_consecutivo = @CodigoConsecutivoActual
                                    )
                                    OR ( @sCo_Emp = saConsecutivo.co_emp
                                         AND saConsecutivo.co_consecutivo = @CodigoConsecutivoActual
                                       ) ) ) 
                    BEGIN
                        INSERT  INTO [saConsecutivo]
                                ( [co_consecutivo], /*[valor],*/ [co_emp], [co_us_in], [fe_us_in], [co_us_mo],
                                  [fe_us_mo] )
                        VALUES
                                ( @CodigoConsecutivoActual, /*0,*/ @sCo_Emp, @sCo_Us_In, GETDATE(), @sCo_Us_In,
                                  GETDATE() )
