# SP: pValidarNotaEntregaVentaCalculos
**Tipo**: Validar
**Módulo**: Ventas
...
## Código (excerpt)
```sql
CREATE PROCEDURE [dbo].[pValidarNotaEntregaVentaCalculos]
    (
...
AS 
	...
	-- Total Bruto (Versus renglones)
        ...
        WHILE @@FETCH_STATUS = 0 
            BEGIN
                SET @PistaMensaje = 'La nota de entrega nro. "' + RTRIM(@pDoc_Num) + '" tiene como valor total bruto "'
                    + LTRIM(RTRIM(STR(@pValorOld, 18, 2))) + '" y el correcto es "' + LTRIM(RTRIM(STR(@pValorNew, 18, 2)))
```
