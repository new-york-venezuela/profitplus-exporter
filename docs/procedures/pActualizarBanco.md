# SP: pActualizarBanco
**Tipo**: Actualizar
**Módulo**: Tesorería

## Tablas Referenciadas
- [`saBanco`](../tables/saBanco.md)

## Código (excerpt)
```sql
/**************************************************************************
*NOMBRE			:	pActualizarBanco
*DESCRIPCIÓN	:	Actualiza un banco
*AUTOR			:	SOFTECH SISTEMAS
***************************************************************************/

CREATE PROCEDURE [dbo].[pActualizarBanco]
    (
      @sCo_Ban CHAR(6) ,
      @sCo_BanOri CHAR(6) ,
      @sDes_Ban VARCHAR(60) ,
      @sTelefonos VARCHAR(60) = NULL ,
      @iPlazo1 INT ,
      @iPlazo2 INT ,
      @iPlazo3 INT ,
      @iPlazo4 INT ,
      @sCampo1 VARCHAR(60) = NULL ,
      @sCampo2 VARCHAR(60) = NULL ,
      @sCampo3 VARCHAR(60) = NULL ,
      @sCampo4 VARCHAR(60) = NULL ,
      @sCampo5 VARCHAR(60) = NULL ,
      @sCampo6 VARCHAR(60) = NULL ,
      @sCampo7 VARCHAR(60) = NULL ,
      @sCampo8 VARCHAR(60) = NULL ,
      @sCo_Us_Mo CHAR(6) ,
      @sCo_Sucu_Mo CHAR(6) = NULL ,
      @sMaquina VARCHAR(60) = NULL ,
      @sCampos VARCHAR(MAX) = NULL ,
      @sRevisado CHAR(1) ,
      @sTrasnfe CHAR(1) ,
      @tsValidador TIMESTAMP ,
      @gRowguid UNIQUEIDENTIFIER = NULL ,
	  @deComisMismoBanco DECIMAL(18,2) = 0,
	  @deComisOtrosBancos DECIMAL(18,2) = 0

    )
AS 
    BEGIN
		
        DECLARE @Tabletimestamp TABLE
            (
              validador VARBINARY(MAX) ,
              fe_us_in DATETIME ,
              fe_us_mo DATETIME ,
              rowguid UNIQUEIDENTIFIER
            )

        UPDATE
            saBanco
        SET co_ban = @sCo_ban, des_ban = @sDes_Ban, telefonos = @sTelefonos, campo1 = @sCampo1, campo2 = @sCampo2,
            campo3 = @sCampo3, campo4 = @sCampo4, campo5 = @sCampo5, campo6 = @sCampo6, campo7 = @sCampo7,
            campo8 = @sCampo8, plazo1 = @iPlazo1, plazo2 = @iPlazo2, plazo3 = @iPlazo3, plazo4 = @iPlazo4,
            co_us_mo = @sCo_Us_Mo, co_sucu_mo = @sCo_Sucu_Mo, fe_us_mo = GETDATE(), revisado = @sRevisado,
            trasnfe = @sTrasnfe, comisMismoBanco = @deComisMismoBanco, comisOtrosBancos = @deComisOtrosBancos
        OUTPUT
            inserted.validador, inserted.fe_us_in, inserted.fe_us_mo, inserted.rowguid
            INTO @Tabletimestamp
        WHERE
            co_ban = @sCo_BanOri
            AND validador = @tsValidador

        DECLARE @dtFe_In DATETIME
        DECLARE @rowGuidOri UNIQUEIDENTIFIER

        SELECT
            @dtFe_In = fe_us_mo, @rowGuidOri = rowguid
        FROM
            @TableTimestamp

        IF @dtFe_In IS NOT NULL 
            BEGIN
			-- Inse
```
