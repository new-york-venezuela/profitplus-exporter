...
-- Description:	<Reporte de Formato de Giros de Compras>
-- =============================================
CREATE PROCEDURE [RepFormatoGiroCompra] 
...
AS 
    BEGIN
        ...
        SELECT
            ( CL.prov_des ) AS cli_des, DV.nro_doc, DV.fec_emis, ROUND(DV.total_neto, 2) AS total_neto,
            DAY(DV.fec_emis) AS dia, MONTH(DV.fec_emis) AS mes, YEAR(DV.fec_emis) AS año, DV.observa,
            dbo.MontoEscrito(ROUND(DV.total_neto, 2)) AS monto_escrito
        FROM
            saProveedor AS CL
            INNER JOIN saDocumentoCompra DV ON ( ( CL.co_prov = DV.co_prov )
