# SP: pExportarActualizarRegistrosConfigPedidoVenta
**Tipo**: Procedimiento
**Módulo**: Ventas
...
```sql
/**************************************************************************
*NOMBRE			:	pExportarActualizarRegistrosConfigPedidoVenta
*DESCRIPCIÓN	:	Actualiza la configuracion del proceso ConfigPedidoVenta
*AUTOR			:	SOFTECH SISTEMAS
***************************************************************************/
CREATE PROCEDURE [dbo].[pExportarActualizarRegistrosConfigPedidoVenta]
    (
      @sCo_Config		CHAR(6) ,
...
AS 
    BEGIN
		...
		SET @sSqlValidador = N'	SELECT @tsValidadorOUT = validador FROM  [' + @sempresa + '].[dbo].[saConfigPedidoVenta] WHERE co_config = @sCo_Config'  
		...
		SET @sSql = N'EXEC [' + @sempresa + '].[dbo].[pActualizarConfigPedidoVenta]
																		@sCo_Config,
																		@sCo_ConfigOri,
																		@sDes_Config,
																		@sCo_Usuario,
