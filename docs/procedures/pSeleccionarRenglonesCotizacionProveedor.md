# SP: pSeleccionarRenglonesCotizacionProveedor
**Tipo**: Seleccionar
**Módulo**: Inventario
...
```sql
/**************************************************************************************************************
NOMBRE			:	pSeleccionarRenglonesCotizacionProveedor
DESCRIPCION		:	Procedimiento para seleccionar los renglones de compra
CREADO POR		:	SOFTECH SISTEMAS
MODIFICADO POR	:	SOFTECH SISTEMAS
***************************************************************************************************************/
CREATE PROCEDURE [pSeleccionarRenglonesCotizacionProveedor] ( @sDoc_Num CHAR(20) )
AS 
    BEGIN 
        SELECT
        ...
        FROM
            saCotizacionProveedorReng f
            INNER JOIN saArticulo a ON ( f.co_art = a.co_art )
            INNER JOIN saCotizacionProveedor E ON E.doc_num = f.doc_num
        WHERE
            f.doc_num = @sDoc_Num
        ORDER BY
            reng_num ASC
