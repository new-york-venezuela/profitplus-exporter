...
-- MODIFICACIÓN: <2020-07-13>
-- Description:	<Reporte de Factura de Ventas por Renglones>
-- =============================================
CREATE PROCEDURE [dbo].[RepFacturaVentaxReng] 
...
-------------------------------
        ...
        SELECT
            @cAnulado AS Filtro_anulado, 'fact' AS tip_rep, ART.art_des, ART.modelo, ART.co_lin, ART.co_subl, ART.co_cat,
            CL.cli_des, VE.ven_des, TR.des_tran, 0.00 AS pend, CP.cond_des,		
		/*Campos saFacturaVenta*/ FV.doc_num, FV.descrip, FV.co_cli, FV.co_tran, FV.co_mone, FV.co_ven, FV.co_cond,
			...
			CONVERT(decimal(18,2),FV.total_neto / ( CASE WHEN @cCo_Moneda IS NULL THEN 1
            ELSE FV.tasa
            END )) AS total_neto, 
			CONVERT(decimal(18,2),FV.saldo / ( CASE WHEN @cCo_Moneda IS NULL THEN 1
            ELSE FV.tasa
            END )) AS saldo, 
			FV.dir_ent,
            FV.comentario, FV.dis_cen, FV.feccom
```
