...
```sql
/************************************************************************
*NOMBRE			: pEliminarMovimientoCaja
*AUTOR			: SOFTECH SISTEMAS
*************************************************************************/

CREATE PROCEDURE [pEliminarMovimientoCaja]
    (
      @sMov_NumOri CHAR(20) ,
        ...
        IF @dtFe_De IS NOT NULL 
			...
			-- Insertar Pista
                EXEC [pInsertarPista] @sUsuario_Id = @sCo_Us_Mo, @dtFecha = @dtFe_De, @sCo_Sucu = @sCo_Sucu_Mo,
                    @sTablaOri = 'saMovimientoCaja', @rowguidOri = @rowGuidOri, @sTipo_Op = 'E', @sMaquina = @sMaquina,
                    @sCampos = @sMov_NumOri
            END
    EN
```
