# SP: pExportarRegistrosConfigOrdenPago
**Tipo**: Procedimiento
**Módulo**: Tesorería
...
```sql
/**************************************************************************
*NOMBRE			:	pExportarRegistrosConfigOrdenPago
*DESCRIPCIÓN	:	Inserta una Configuración en la tabla saConfigOrdenPago
*AUTOR			:	SOFTECH SISTEMAS
***************************************************************************/
CREATE PROCEDURE [dbo].[pExportarRegistrosConfigOrdenPago]
    (
      @sCo_Config CHAR(6) ,
...
AS 
    BEGIN
		...
		SET @sSql = N'EXEC [' + @sempresa + '].[dbo].[pInsertarConfigOrdenPago]	
																		@sCo_Config,
																		@sDes_Config,
																		@sCo_Usuario,
																		@sCo_Mapa,
