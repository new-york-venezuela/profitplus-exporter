...
-- Description:	<Información Contable Orden de Pago>
-- =============================================
CREATE PROCEDURE [dbo].[RepInfContableOrdenPago]
	@sNumero_d CHAR(20) = NULL,
	@sNumero_h CHAR(20) = NULL,
	@dFecha_d SMALLDATETIME = NULL,
    @dFecha_h SMALLDATETIME = NULL,
	@scod_ben_d CHAR(16) = NULL,
    @scod_ben_h CHAR(16) = NULL,
...
BEGIN
  ...
  (Select A.ord_num, ORD.fecha, ORD.cod_ben, (select ben_des from saBeneficiario where cod_ben = ORD.cod_ben) as ben_des, 
  ORD.feccom, ORD.numcom, A.num_carp, A.Descripcion, case when ltrim(A.monto) = '' then 0 else cast(A.monto as decimal(18,5)) end as monto, 
  A.co_cue, A.co_gas, A.co_cen, A.des_cen, case when ltrim(A.porc) = '' then 0 else cast(A.porc as decimal(18,5)) end as porc,
  'Encabezado' as tipo, 0 as reng_num, '' as co_cta_ingr_egr, '' as des_cta,   
  ORD.descrip,
  CUE.des_cue, 
  GAS.des_gas
 From (

 Select	A.ord_num, A.feccom, '01' as num_carp,
