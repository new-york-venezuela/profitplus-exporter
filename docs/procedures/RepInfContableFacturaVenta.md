...
-- Description:	<Información Contable de Facturas de Ventas>
-- =============================================
CREATE PROCEDURE [dbo].[RepInfContableFacturaVenta]
...
BEGIN
  ...
  (Select A.doc_num, VEN.fec_emis, VEN.co_cli, (select cli_des from saCliente where co_cli = VEN.co_cli) as cli_des, VEN.co_ven, 
  VEN.feccom, VEN.numcom, A.num_carp, A.Descripcion, case when ltrim(A.monto) = '' then 0 else cast(A.monto as decimal(18,5)) end as monto, 
  A.co_cue, A.co_gas, A.co_cen, A.des_cen, case when ltrim(A.porc) = '' then 0 else cast(A.porc as decimal(18,5)) end as porc, 
  'Encabezado' as tipo, 0 as reng_num, '' as co_art, '' as des_art,
  VEN.descrip, 
  CUE.des_cue, 
  GAS.des_gas
 From (
 ...
 Select	A.doc_num, A.feccom, '01' as num_carp,
		...
		(select des_cen from scCentro where co_cen=C.N
```
