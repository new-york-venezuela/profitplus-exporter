# SP: pvpInsertarEtiquetaBalanza
**Tipo**: Punto de Venta
**Módulo**: Punto de Venta

## Tablas Referenciadas
- [`pvEtiquetaBalanza`](../tables/pvEtiquetaBalanza.md)

## Código (excerpt)
```sql
/************************************************************************
*NOMBRE			: pvpInsertarEtiquetaBalanza
*DESCRIPCIÓN	: Inserta una etiqueta para Balanza
*AUTOR			: SOFTECH SISTEMAS
*CREACIÓN		: 09/09/2013
*ACTUALIZACIÓN	: 16/09/2020
*************************************************************************/ 
CREATE PROCEDURE [dbo].[pvpInsertarEtiquetaBalanza]
    (
      @sCo_Etiqueta		CHAR(6) ,
      @sDes_Etiqueta	VARCHAR(60),
      @iPre				INTEGER ,
      @iCod				INTEGER ,
      @iSuf				INTEGER ,
      @iEnt				INTEGER ,
      @iDec				INTEGER ,
      @iAdic			INTEGER ,
      @bActivo			BIT,
      @sCampo1			VARCHAR(60) = NULL ,
      @sCampo2			VARCHAR(60) = NULL ,
      @sCampo3			VARCHAR(60) = NULL ,
      @sCampo4			VARCHAR(60) = NULL ,
      @sCampo5			VARCHAR(60) = NULL ,
      @sCampo6			VARCHAR(60) = NULL ,
      @sCampo7			VARCHAR(60) = NULL ,
      @sCampo8			VARCHAR(60) = NULL ,
      @sCo_Us_In		CHAR(6) ,
      @sCo_Sucu_In		CHAR(6),
      @sMaquina			VARCHAR(60) = NULL ,
      @sRevisado		CHAR(1),
      @sTrasnfe			CHAR(1)	  
-->>JN 20200826
	,@sPeso_precio		CHAR(2) = NULL
--<<JN 20200826 
      
    )
AS 
    BEGIN

        DECLARE @TableTimestamp TABLE
            (
              validador		VARBINARY(MAX) ,
              fe_us_in		DATETIME ,
              fe_us_mo		DATETIME ,
              rowguid		UNIQUEIDENTIFIER
            )

        INSERT  INTO pvEtiquetaBalanza
				( co_etiqueta, des_etiqueta, pre, cod, suf, ent, DEC, adic, activo,
				  campo1, campo2, campo3, campo4, campo5, campo6, campo7, campo8,
                  co_us_in, fe_us_in, co_us_mo, fe_us_mo, revisado, trasnfe, co_sucu_in, co_sucu_mo
				  -->>JN 20200826
				  ,peso_precio
				  --<<JN 20200826
				   )
        OUTPUT  Inserted.validador, inserted.fe_us_in, inserted.fe_us_mo, Inserted.rowguid
                INTO @TableTimestamp
        VALUES
                ( @sCo_Etiqueta, @sDes_etiqueta, @ipre, @icod, @isuf, @ient, @idec,@iadic, @bActivo,
				  @sCampo1, @sCampo2, @sCampo3, @sCampo4, @sCampo5, @sCampo6,@sCampo7, @sCampo8,
			      @sCo_Us_In, GETDATE(), @sCo_Us_In, GETDATE(), @sRevisado, @sTrasnfe, @sCo_Sucu_In,
                  @sCo_Sucu_In 
				  -->>JN 20200826
				  ,@sPeso_precio
				  --<<JN 20200826
				  )

        DECLARE @dtFe_In DATETIME
        DECLARE @rowGuidOri UNIQUEIDENTIFIER

        SELECT
            @dtFe_In = fe_us_in, @rowGuidOri = rowguid
        FROM
```
