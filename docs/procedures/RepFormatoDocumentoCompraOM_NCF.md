# SP: RepFormatoDocumentoCompraOM_NCF
**Tipo**: Reporte
**Módulo**: Compras
...
-- Description:	Reporte para Documento de Compra NCF (Dominicana) en Otra Moneda.
-- =============================================
CREATE PROCEDURE [dbo].[RepFormatoDocumentoCompraOM_NCF] 
...
AS 
    BEGIN
        ...
        SELECT
             'docucompra'  AS TIPO_DOC, 'tipo' = 0, ( CL.prov_des ) AS cli_des,DV.nro_fact, CL.rif, CL.telefonos, CL.direc1, CL.direc2,CL.campo1 as camp, --VE.ven_des, 
            DV.co_tipo_doc,TD.descrip,DV.nro_doc, DV.nro_doc, ( DV.co_prov ) AS co_cli, /*DV.co_ven,*/ DV.co_mone, DV.tasa, DV.observa,
            DV.fec_emis, DV.fec_venc, DV.doc_orig, DV.nro_orig, DV.n_control, DV.total_bruto / DV.tasa AS total_bruto,
            DV.monto_imp / DV.tasa AS monto_imp, DV.monto_desc_glob / DV.tasa AS monto_desc_glob, DV.porc_desc_glob,
            DV.monto_reca / DV.tasa AS monto_reca, DV.porc_reca,CL.fax,NCF.ncf,G.co_gasto,G.des_tipo,C.co_tipo,C.des_tipo, CONVERT(char,SEXT.fe_venc,103)as fe_venc,CL.direc1, 
            ( ( DV.otros1 / DV.tasa ) + ( DV.otros2 / DV.tasa ) + ( DV.otros3 / DV.tasa ) ) AS otros, 
            null fec_emis_ori, null total_bruto_ori, null monto_imp_ori, null total_neto_ori, NCF.tipo_doc_Ori , NCF.nro_doc_Ori, NCFF.ncf as ncf_afectado			
        FROM
