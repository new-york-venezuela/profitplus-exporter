# SP: pValidarRengImportadoCotizacionProveedorReng
**Tipo**: Validar
**Módulo**: Compras
...
## Código (excerpt)
```sql
CREATE PROCEDURE [pValidarRengImportadoCotizacionProveedorReng]
    (
...
AS 
    BEGIN	
        ...
        FOR
            ...
            FROM
                saFacturaCompraReng R1
                LEFT JOIN saCotizacionProveedorReng R2 ON R1.rowguid_doc = R2.rowguid
            WHERE
        ...
        WHILE @@FETCH_STATUS = 0 
            BEGIN
                ...
                IF @pR2Reng_num IS NULL 
                    BEGIN
                        SET @PistaMensaje = 'La factura de compra nro. "' + RTRIM(@pR1Doc_Num) + '" renglon "'
                            + LTRIM(RTRIM(STR(@pR1Reng_num))) + '" viene de un renglon que no existe.' 
                        SET @PuedeCorregir = @PuedeCorregirZoom
```
