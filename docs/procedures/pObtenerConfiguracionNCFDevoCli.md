# SP: pObtenerConfiguracionNCFDevoCli
**Tipo**: Obtener
**Módulo**: General
...
-- =============================================
--EXEC pObtenerConfiguracionNCFDevoCli 'NCF14', 'PROFIT', '300'

CREATE PROCEDURE [dbo].[pObtenerConfiguracionNCFDevoCli]
    (
      @sTipo VARCHAR(max),
	  @cUsuario CHAR(6),
	  @cMapa CHAR(6)
...
AS 
    BEGIN
		...
		SET @Expresion = 'SELECT contr.C.value(''@Habilitado'', ''Varchar(5)'') AS habilitado, ' + 
			...
			'saConfigDevolucionCliente CROSS APPLY xml_reglas.nodes(''/Reglas/Adicional/IfCombo/' + @sTipo + ''') contr ( c ) '
