# SP: pValidarDevolucionClienteCalculos
**Tipo**: Validar
**Módulo**: Ventas
...
## Código (excerpt)
```sql
CREATE PROCEDURE [dbo].[pValidarDevolucionClienteCalculos]
    (
...
AS 
	...
	-- Total Bruto (Versus renglones)
        ...
        WHILE @@FETCH_STATUS = 0 
            BEGIN
                SET @PistaMensaje = 'La devolución a cliente nro. "' + RTRIM(@pDoc_Num)
                    + '" tiene como valor total bruto "' + LTRIM(RTRIM(STR(@pValorOld, 18, 2))) + '" y el correcto es "'
                    + LTRIM(RTRIM(STR(@pVal
```
