# SP: RepTotalNotaRecepcionEmpresa
**Tipo**: Reporte
**Módulo**: Compras
...
-- Description:	<Reporte de Total de Notas de Recepcion de la Empresa>
-- =============================================
CREATE PROCEDURE [RepTotalNotaRecepcionEmpresa]
...
AS 
    BEGIN
        ...
        SELECT
            ...
            NR.trasnfe, NR.validador, NR.rowguid, NRR.coti,
            ROUND(( NRR.cost_vta - NRR.monto_desc - NRR.monto_desc_glob + NRR.monto_reca_glob ), 2) AS total_bruto
        FROM
            saNotaRecepcionCompra AS NR --INNER JOIN saNotaRecepcionCompraReng AS NRR ON NRR.doc_num = NR.doc_num
            INNER JOIN ( SELECT DISTINCT
                            ...
                            SUM(otros3_glob) AS otros3,
                            SUM(( ( total_art - pendiente ) * ( (
```
