# SP: pValidarPendientePlantillaVentaReng
**Tipo**: Validar
**Módulo**: General
...
## Código (excerpt)
```sql
CREATE PROCEDURE [pValidarPendientePlantillaVentaReng]
    (
...
AS 
    BEGIN	
        ...
        WHILE @@FETCH_STATUS = 0 
            BEGIN
                SET @PistaMensaje = 'La plantilla de venta nro. "' + RTRIM(@pDoc_Num) + '" renglon "'
                ...
                IF ( @bCorregir = 1 ) 
                    BEGIN
                        ...
                        EXEC [pInsertarPista] @sUsuario_Id = 'VALCON', @dtFecha = @HoraCorrida, @sCo_Sucu = NULL,
                            @sTablaOri = 'saPlantillaVentaReng', @rowguidOri = @Id, @sTipo_Op = N'M', @sMaquina = NULL,
                ...
                SET @HoraCorrida = GETDATE()
                EXEC [pInsertarPista] @sUsuario_Id = 'VALCON', @dtFecha = @HoraCorrida, @sCo_Sucu = NULL,
                    @sTablaOri = 'saPlantillaVentaReng', @rowguidOri = @IdProcess, @sTipo_Op = N'M', @sMaquina = NULL,
                    @sCampos = @PistaMensaje
