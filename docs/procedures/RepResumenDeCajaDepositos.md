...
```sql
/**************************************************************************************************************
NOMBRE: [RepResumenDeCajaDepositos]
DESCRIPCION: Reporte de Movimientos de Caja por Turno
CREADO POR: SOFTECH SISTEMAS
***************************************************************************************************************/
CREATE PROCEDURE [dbo].[RepResumenDeCajaDepositos]
...
AS 
    BEGIN   
		 ...
		 De.origen, De.cod_cta, cb.num_cta, De.cob_pag cobro
  FROM   pvturnoexe as pv
     ...
     WHERE   
        ...
        AND (@dFecha_h IS NULL     OR dbo.FechaSimple(De.fecha) <= dbo.FechaSimple(@dFecha_h))
		AND (De.origen <> 'TRA')            
		AND (@sStatus is null      OR pv.statu
```
