...
## Código (excerpt)
```sql
CREATE PROCEDURE [pValidarStockDes]
...
AS 
    BEGIN
        ...
        FOR
            ...
            FROM
                ( SELECT
                    co_art, co_alma, SUM(total_art) AS total_art, 'DES' AS tipo_stock
                  FROM
                    ( SELECT
                        *
                      FROM
                ...
                FULL OUTER JOIN saStockAlmacen ON TABLA_CONSOLIDADA_STOCK_DES.co_alma = saStockAlmacen.co_alma
                                                  ...
                                                  AND ( TABLA_CONSOLIDADA_STOCK_DES.tipo_stock = 'DES'
                                                        OR saStockAlmacen.tipo = 'DES'
                                                      )
            WHERE
                ( TABLA_CONSOLIDADA_STOCK_DES.tipo_stock = 'DES'
                  OR saStockAlmacen.tipo = 'DES'
                )
                AND ( TABLA_CONSOLIDADA_STOCK_DES.total_art <> saStockAlmacen.stock
                      OR ( TABLA_CONSOLIDADA_STOCK_DES.total_art IS NULL
                           AND saStockAlmacen.s
```
