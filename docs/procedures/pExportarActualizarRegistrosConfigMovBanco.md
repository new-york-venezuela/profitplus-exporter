# SP: pExportarActualizarRegistrosConfigMovBanco
**Tipo**: Procedimiento
**Módulo**: Tesorería
...
```sql
/**************************************************************************
*NOMBRE			:	pExportarActualizarRegistrosConfigMovBanco
*DESCRIPCIÓN	:	Actualiza la configuracion del proceso ConfigMovBanco
*AUTOR			:	SOFTECH SISTEMAS
***************************************************************************/
CREATE PROCEDURE [dbo].[pExportarActualizarRegistrosConfigMovBanco]
    (
      @sCo_Config		CHAR(6) ,
...
AS 
    BEGIN
		...
		SET @sSqlValidador = N'	SELECT @tsValidadorOUT = validador FROM  [' + @sempresa + '].[dbo].[saConfigMovBanco] WHERE co_config = @sCo_Config'  
		...
		SET @sSql = N'EXEC [' + @sempresa + '].[dbo].[pActualizarConfigMovBanco]
																		@sCo_Config,
																		@sCo_ConfigOri,
																		@sDes_Config,
																		@sCo_Usuario,
