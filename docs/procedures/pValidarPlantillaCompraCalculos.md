# SP: pValidarPlantillaCompraCalculos
**Tipo**: Validar
**Módulo**: General
...
## Código (excerpt)
```sql
CREATE PROCEDURE [dbo].[pValidarPlantillaCompraCalculos]
    (
...
AS 
	...
	-- Total Bruto (Versus renglones)
        ...
        FOR
            ...
            FROM
                saPlantillaCompra E
                INNER JOIN saPlantillaCompraReng R ON e.doc_num = R.doc_num
            GROUP BY
        ...
        WHILE @@FETCH_STATUS = 0 
            BEGIN
                SET @PistaMensaje = 'La plantilla de compra nro. "' + RTRIM(@pDoc_Num)
                    + '" tiene como valor total bruto "' + LTRIM(RTRIM(STR(@pValorOld, 18, 2))) + '" y el correcto es "'
                    + LTRIM(RTRIM(STR(@pValorNew,
```
