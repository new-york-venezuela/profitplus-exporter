# SP: pExportarActualizarRegistrosConfigDistCosto
**Tipo**: Procedimiento
**Módulo**: General
...
```sql
/**************************************************************************
*NOMBRE			:	pExportarActualizarRegistrosConfigDistCosto
*DESCRIPCIÓN	:	Actualiza la configuracion del proceso ConfigDistCosto
*AUTOR			:	SOFTECH SISTEMAS
***************************************************************************/
CREATE PROCEDURE [dbo].[pExportarActualizarRegistrosConfigDistCosto]
    (
      @sCo_Config		CHAR(6) ,
...
AS 
    BEGIN
		...
		SET @sSqlValidador = N'	SELECT @tsValidadorOUT = validador FROM  [' + @sempresa + '].[dbo].[saConfigDistCosto] WHERE co_config = @sCo_Config'  
		...
		SET @sSql = N'EXEC [' + @sempresa + '].[dbo].[pActualizarConfigDistCosto]
																		@sCo_Config,
																		@sCo_ConfigOri,
																		@sDes_Config,
																		@sCo_Usuario,
