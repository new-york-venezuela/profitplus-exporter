# SP: pSeleccionarContabilizacionNotaDebitoVenta
**Tipo**: Seleccionar
**Módulo**: Ventas
...
## Código (excerpt)
```sql
CREATE PROCEDURE [dbo].[pSeleccionarContabilizacionNotaDebitoVenta]
    (
...
AS 
	 ...
	 */
		...
		--(RTRIM(DV.co_tipo_doc) + 
            ( RTRIM(DV.nro_doc) ) AS Co_Doc, CL.co_cli AS Co_Auxiliar, CL.cli_des AS Des
```
