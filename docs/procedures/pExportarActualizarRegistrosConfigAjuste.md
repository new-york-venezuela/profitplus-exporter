# SP: pExportarActualizarRegistrosConfigAjuste
**Tipo**: Procedimiento
**Módulo**: Inventario
...
```sql
/**************************************************************************
*NOMBRE			:	pExportarActualizarRegistrosConfigAjuste
*DESCRIPCIÓN	:	Actualiza la configuracion del proceso ConfigAjuste
*AUTOR			:	SOFTECH SISTEMAS
***************************************************************************/
CREATE PROCEDURE [dbo].[pExportarActualizarRegistrosConfigAjuste]
    (
      @sCo_Config		CHAR(6) ,
...
AS 
    BEGIN
		...
		SET @sSqlValidador = N'	SELECT @tsValidadorOUT = validador FROM  [' + @sempresa + '].[dbo].[saConfigAjuste] WHERE co_config = @sCo_Config'  
		...
		SET @sSql = N'EXEC [' + @sempresa + '].[dbo].[pActualizarConfigAjuste]
																		@sCo_Config,
																		@sCo_ConfigOri,
																		@sDes_Config,
																		@sCo_Usuario,
