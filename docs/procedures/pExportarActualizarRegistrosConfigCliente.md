# SP: pExportarActualizarRegistrosConfigCliente
**Tipo**: Procedimiento
**Módulo**: Clientes
...
```sql
/**************************************************************************
*NOMBRE			:	pExportarActualizarRegistrosConfigCliente
*DESCRIPCIÓN	:	Actualiza la configuracion del proceso ConfigCliente
*AUTOR			:	SOFTECH SISTEMAS
***************************************************************************/
CREATE PROCEDURE [dbo].[pExportarActualizarRegistrosConfigCliente]
    (
      @sCo_Config		CHAR(6) ,
...
AS 
    BEGIN
		...
		SET @sSqlValidador = N'	SELECT @tsValidadorOUT = validador FROM  [' + @sempresa + '].[dbo].[saConfigCliente] WHERE co_config = @sCo_Config'  
		...
		SET @sSql = N'EXEC [' + @sempresa + '].[dbo].[pActualizarConfigCliente]
																		@sCo_Config,
																		@sCo_ConfigOri,
																		@sDes_Config,
																		@sCo_Usuario,
