...
-- Description:	<Reporte de Plantillas de Ventas por Número>
-- =============================================
CREATE PROCEDURE [RepPlantillaVentaxNum] 
...
AS 
    BEGIN
        ...
        IF @dCo_fecha_h IS NOT NULL 
            SET @dCo_fecha_h = dbo.FechaSimple(@dCo_fecha_h)-- Insert statements for procedure here
...
-------------------------------
        SELECT
            @cAnulado AS Filtro_anulado, 'PLANTI' AS tipo_rep, CLI.tip_cli, CLI.cli_des, CLI.co_zon,
		/*Campos saFacturaVenta*/ FV.doc_num, FV.descrip, FV.co_cli, FV.co_tran, FV.co_mone, FV.co_ven, FV.co_cond,
            ...
            FV.porc_desc_glob, FV.monto_desc_glob, FV.porc_reca, 
			FV.monto_reca, FV.total_bruto, 
			FV.monto_imp/ ( CASE WHEN @cCo_Moneda IS NULL THEN 1
                                   ELSE FV.tasa
                              END ) AS monto_imp,
