# SP: pExportarActualizarRegistrosConfigFacturaCompra
**Tipo**: Procedimiento
**Módulo**: Compras
...
```sql
/**************************************************************************
*NOMBRE			:	pExportarActualizarRegistrosConfigFacturaCompra
*DESCRIPCIÓN	:	Actualiza la configuracion del proceso ConfigFacturaCompra
*AUTOR			:	SOFTECH SISTEMAS
***************************************************************************/
CREATE PROCEDURE [dbo].[pExportarActualizarRegistrosConfigFacturaCompra]
    (
      @sCo_Config		CHAR(6) ,
...
AS 
    BEGIN
		...
		SET @sSqlValidador = N'	SELECT @tsValidadorOUT = validador FROM  [' + @sempresa + '].[dbo].[saConfigFacturaCompra] WHERE co_config = @sCo_Config'  
		...
		SET @sSql = N'EXEC [' + @sempresa + '].[dbo].[pActualizarConfigFacturaCompra]
																		@sCo_Config,
																		@sCo_ConfigOri,
																		@sDes_Config,
																		@sCo_Usuario,
