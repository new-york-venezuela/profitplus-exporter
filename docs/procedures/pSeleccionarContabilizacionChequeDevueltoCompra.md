# SP: pSeleccionarContabilizacionChequeDevueltoCompra
**Tipo**: Seleccionar
**Módulo**: Tesorería
...
## Código (excerpt)
```sql
CREATE PROCEDURE [pSeleccionarContabilizacionChequeDevueltoCompra]
   (
...
AS 
	...
	--DECLARE @tiene_reng BIT   
	--   SET @tiene_reng = (SELECT (CASE WHEN COUNT(co_tipo_doc) > 0 THEN 1 ELSE 0 END) FROM saDocumentoComp
```
