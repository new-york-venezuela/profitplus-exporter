# SP: RepFormatoNotaEntregaVenta
**Tipo**: Reporte
**Módulo**: Ventas

## Tablas Referenciadas
- [`par_emp`](../tables/par_emp.md)
- [`saAdiCampo`](../tables/saAdiCampo.md)
- [`saAlmacen`](../tables/saAlmacen.md)
- [`saArticulo`](../tables/saArticulo.md)
- [`saCliente`](../tables/saCliente.md)
- [`saCondicionPago`](../tables/saCondicionPago.md)
- [`saFacturaVenta`](../tables/saFacturaVenta.md)
- [`saFacturaVentaReng`](../tables/saFacturaVentaReng.md)
- [`saMoneda`](../tables/saMoneda.md)
- [`saNotaEntregaVenta`](../tables/saNotaEntregaVenta.md)
- [`saNotaEntregaVentaReng`](../tables/saNotaEntregaVentaReng.md)
- [`saTransporte`](../tables/saTransporte.md)
- [`saVendedor`](../tables/saVendedor.md)

## Código (excerpt)
```sql
-- =============================================
-- Author:		SOFTECH SISTEMAS
-- Create date: <25/08/2010>
-- Description:	<Formato de Notas de Entrega de Venta >
-- =============================================
CREATE PROCEDURE [dbo].[RepFormatoNotaEntregaVenta] 
	-- Add the parameters for the stored procedure here
    @cCo_Numero_d CHAR(20) = NULL ,
    @cCo_Numero_h CHAR(20) = NULL ,
    @cCo_Sucursal CHAR(6) = NULL ,
    @sCampOrderBy VARCHAR(16) = NULL ,
    @sDir VARCHAR(6) = NULL ,
    @bHeaderRep BIT = 0
AS 
    BEGIN
        SET NOCOUNT ON ;

    -- Insert statements for procedure here
        DECLARE @Tipo_doc CHAR(11) ;
        SET @Tipo_doc = 'notaeventa' ;
		DECLARE @countAlmacenes INT;

		SELECT @countAlmacenes = COUNT(DISTINCT FVR.co_alma)
		FROM saNotaEntregaVenta FV
		INNER JOIN saNotaEntregaVentaReng FVR ON FVR.doc_num = FV.doc_num
		WHERE 
			( ( @cCo_Numero_d IS NULL OR FV.doc_num >= @cCo_Numero_d ) )
			AND ( ( @cCo_Numero_h IS NULL OR FV.doc_num <= @cCo_Numero_h ) )
			AND ( FV.anulado = 0 )
			AND ( @cCo_Sucursal IS NULL OR @cCo_Sucursal = FV.co_sucu_in );

			
			-- Validar si hay más de un almacén diferente
			IF @countAlmacenes > 1
			BEGIN
				RAISERROR ( 'No se puede imprimir el formato, la Orden de Entrega contiene más de un almacen.', 16, 1);
				RETURN; 
			END

		declare @DirFis as nvarchar(254)
		declare @Telef as nvarchar(254)
        declare @MonedaBase as char(6)
		select @MonedaBase=g_moneda from par_emp 
		select @DirFis=val_str from saAdiCampo where co_adicampo = 'dir_fis'
		select @Telef=val_str from saAdiCampo where co_adicampo ='telef'
	
        SELECT
            @Tipo_doc AS TIPO_DOC, 'tipo' = 1, ART.modelo, CL.cli_des, CL.rif, CL.nit, CL.telefonos, CL.fax, CL.direc1,
            (CASE WHEN (FV.dir_ent IS NOT NULL AND len(ltrim(FV.dir_ent)) > 0) THEN FV.dir_ent ELSE CL.dir_ent2 END) AS dir_entrega,
            VE.ven_des, TR.des_tran, CP.cond_des, MO.mone_des,
		/*Campos saFacturaVenta*/ FV.doc_num, FV.descrip, FV.co_cli, FV.co_tran, FV.co_mone, FV.co_ven, FV.co_cond,
            FV.fec_emis, FV.fec_venc, FV.fec_reg, FV.anulado, FV.status, FV.n_control, FV.ven_ter, FV.tasa,
            FV.porc_desc_glob, FV.monto_desc_glob, FV.porc_reca, FV.monto_reca, FV.total_bruto, FV.monto_imp,
            FV.monto_imp2, FV.monto_imp3, FV.otros1, FV.otros2, FV.otros3, FV.total_neto, FV.saldo, FV.dir_ent,
            FV.comentario, FV.dis_cen, FV.feccom, FV.numcom, FV.contr
```
