# SP: pValidarChequeDevueltoMovBancoCompra
**Tipo**: Validar
**Módulo**: Compras
...
-- =============================================
CREATE PROCEDURE [dbo].[pValidarChequeDevueltoMovBancoCompra]
	(
...
AS
	BEGIN
		...
		5ta parte
		*/
		--Valida que el moviento de banco tenga asociado un documento de compra de tipo (CHEQ)
			SELECT DISTINCT
				'El movimiento de banco "' +  RTRIM(MB.mov_num) +  '" no posee documento de Compra asociado' 
			...
			FROM
				...
				LEFT JOIN saDocumentoCompra DV ON DV.mov_ban = MB.mov_num
			WHERE 
				MB.mov_num NOT IN (Select mov_ban from saDocumentoCompra where mov_ban IS NOT NULL) AND origen ='CHD' AND cob_pag IS NOT NULL AND tipo_op = 'NC'
		...
		WHILE @@FETCH_STATUS = 0
			...
			BEGIN
				 IF @Valor = 1
					...
					VALUES (@PistaMensaje)
					FETCH NEXT FROM PENDIENTE_VALIDAR INTO @Moti
```
