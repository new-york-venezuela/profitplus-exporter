# SP: pExportarRegistrosConfigDistCosto
**Tipo**: Procedimiento
**Módulo**: General
...
```sql
/**************************************************************************
*NOMBRE			:	pExportarRegistrosConfigDistCosto
*DESCRIPCIÓN	:	Inserta una Configuración en la tabla saConfigDistCosto
*AUTOR			:	SOFTECH SISTEMAS
***************************************************************************/
CREATE PROCEDURE [dbo].[pExportarRegistrosConfigDistCosto]
    (
      @sCo_Config CHAR(6) ,
...
AS 
    BEGIN
		...
		SET @sSql = N'EXEC [' + @sempresa + '].[dbo].[pInsertarConfigDistCosto]	
																		@sCo_Config,
																		@sDes_Config,
																		@sCo_Usuario,
																		@sCo_Mapa,
