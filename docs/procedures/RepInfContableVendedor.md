...
-- Description:	<Información Contable de Vendedor>
-- =============================================
CREATE PROCEDURE [dbo].[RepInfContableVendedor]
...
BEGIN
	SET NOCOUNT ON;
   Select A.co_ven, A.num_carp, A.Descripcion, case when ltrim(A.monto) = '' then 0 else cast(A.monto as decimal(18,5)) end as monto,
   ...
   Select	A.co_ven, '01' as num_carp,
	...
	Where 
		((@sCo_Ven_d IS NULL OR A.co_ven >= @sCo_Ven_d)		AND ( @sCo_Ven_h IS NULL OR A.co_ven <= @sCo_ven_h))
...
UNION ALL
	...
	select	A.co_ven, '02' as num_carp,
