...
-- Description:	<Información Contable de Transporte>
-- =============================================
CREATE PROCEDURE [dbo].[RepInfContableTransporte]
	@sCo_Tran_d CHAR(6) = NULL,
	@sCo_Tran_h CHAR(6) = NULL,
...
BEGIN
	SET NOCOUNT ON;
   Select A.co_tran, A.num_carp, A.Descripcion, case when ltrim(A.monto) = '' then 0 else cast(A.monto as decimal(18,5)) end as monto,
   ...
   Select	A.co_tran, '01' as num_carp,
	...
	Where 
		((@sCo_Tran_d IS NULL OR A.co_tran >= @sCo_Tran_d)		AND ( @sCo_Tran_h IS NULL OR A.co_tran <= @sCo_Tran_h))
...
UNION ALL
	...
	select	A.co_tran, '02' as num_carp,
		...
		A.dis_cen.query( '/InformacionContable/Carpeta02/CuentaContable').value('/','varchar(20)
```
