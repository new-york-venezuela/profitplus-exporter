# SP: pSeleccionarRenglonesNotaDespachoVenta
**Tipo**: Seleccionar
**Módulo**: Inventario
...
DESCRIPCION: Selecciona los renglones de un Despacho de venta
CREADO POR: SOFTECH SISTEMAS
***************************************************************************************************************/
CREATE PROCEDURE [pSeleccionarRenglonesNotaDespachoVenta] ( @sDoc_Num CHAR(20) )
AS 
    BEGIN 
        SELECT
        ...
        FROM
            saNotaDespachoVentaReng f
            INNER JOIN saArticulo a ON ( f.co_art = a.co_art )
            INNER JOIN saNotaDespachoVenta E ON E.doc_num = f.doc_num
        WHERE
            f.doc_num = @sDoc_Num
        ORDER BY
            reng_num ASC
