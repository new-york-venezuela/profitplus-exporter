...
-- Description:	<Información Contable de Beneficiario>
-- =============================================
CREATE PROCEDURE [dbo].[RepInfContableBeneficiario]
	@sCod_Ben_d CHAR(10) = NULL,
	@sCod_Ben_h CHAR(10) = NULL,
...
BEGIN
	SET NOCOUNT ON;
   Select A.cod_ben, A.num_carp, A.Descripcion, case when ltrim(A.monto) = '' then 0 else cast(A.monto as decimal(18,5)) end as monto,
   ...
   Select	A.cod_ben, '01' as num_carp,
	...
	from	saBeneficiario A
		outer apply A.dis_cen.nodes('InformacionContable/Carpeta01/DistribucionCentros/CentroCosto') as C(N)
	Where 
		((@sCod_Ben_d IS NULL OR A.cod_ben >= @sCod_Ben_d) AND (@sCod_Ben_h IS NULL OR A.cod_ben <= @sCod_Ben_h))
...
UNION ALL
	...
	select	A.cod_ben, '02' as num_carp,
