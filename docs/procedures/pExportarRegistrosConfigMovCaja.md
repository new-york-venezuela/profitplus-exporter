# SP: pExportarRegistrosConfigMovCaja
**Tipo**: Procedimiento
**Módulo**: Tesorería
...
```sql
/**************************************************************************
*NOMBRE			:	pExportarRegistrosConfigMovCaja
*DESCRIPCIÓN	:	Inserta una Configuración en la tabla saConfigMovCaja
*AUTOR			:	SOFTECH SISTEMAS
***************************************************************************/
CREATE PROCEDURE [dbo].[pExportarRegistrosConfigMovCaja]
    (
      @sCo_Config CHAR(6) ,
...
AS 
    BEGIN
		...
		SET @sSql = N'EXEC [' + @sempresa + '].[dbo].[pInsertarConfigMovCaja]	
																		@sCo_Config,
																		@sDes_Config,
																		@sCo_Usuario,
																		@sCo_Mapa,
