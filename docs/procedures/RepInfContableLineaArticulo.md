# SP: RepInfContableLineaArticulo
**Tipo**: Reporte
**Módulo**: Inventario
...
-- Description:	<Información Contable de Linea con Artículos>
-- =============================================
CREATE PROCEDURE [dbo].[RepInfContableLineaArticulo]
...
BEGIN
	SET NOCOUNT ON;
  Select A.co_lin, A.num_carp, A.Descripcion, case when ltrim(A.monto) = '' then 0 else cast(A.monto as decimal(18,5)) end as monto, 
...
Select	A.co_lin, '01' as num_carp,
...
select	A.co_lin, '02' as num_carp,
		...
		A.dis_cen.query( '/InformacionContable/Carpeta02/CuentaContable').value('
```
