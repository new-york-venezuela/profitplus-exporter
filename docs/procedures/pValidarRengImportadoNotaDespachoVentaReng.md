# SP: pValidarRengImportadoNotaDespachoVentaReng
**Tipo**: Validar
**Módulo**: Ventas
...
## Código (excerpt)
```sql
CREATE PROCEDURE [pValidarRengImportadoNotaDespachoVentaReng]
    (
...
AS 
    BEGIN	
        ...
        FOR
            ...
            FROM
                saDevolucionClienteReng R1
                LEFT JOIN saNotaDespachoVentaReng R2 ON R1.rowguid_doc = R2.rowguid
            WHERE
        ...
        WHILE @@FETCH_STATUS = 0 
            BEGIN
                ...
                IF @pR2Reng_num IS NULL 
                    BEGIN
                        SET @PistaMensaje = 'La devolucion de cliente nro. "' + RTRIM(@pR1Doc_Num) + '" renglon "'
                            + LTRIM(RTRIM(STR(@pR1Reng_num))) + '" viene de un renglon que no existe.' 
                        SET @PuedeCorregir = @PuedeCorregir
```
