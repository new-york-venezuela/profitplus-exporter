...
```sql
/**********************************************************************************************
*NOMBRE			:	pObtenerDocumentoDeposito
...
**********************************************************************************************/
CREATE PROCEDURE [dbo].[pObtenerDocumentoDeposito] ( @sCodCaja VARCHAR(6) )
AS 
    BEGIN
        ...
        FROM
            saCaja c
            INNER JOIN saMovimientoCaja mc ON c.cod_caja = mc.cod_caja
            LEFT JOIN saTarjetaCredito tc ON mc.co_tar = tc.co_tar
			LEFT JOIN pvValeAlimentacion va ON va.co_vale = mc.co_vale 
            INNER JOIN saCaja sc ON sc.cod_caja
```
