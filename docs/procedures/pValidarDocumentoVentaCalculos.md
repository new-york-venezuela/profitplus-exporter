# SP: pValidarDocumentoVentaCalculos
**Tipo**: Validar
**Módulo**: Fiscal
...
-- Description:	pValidarDocumentoVentaCalculos
-- =============================================
CREATE PROCEDURE [dbo].[pValidarDocumentoVentaCalculos]
    (
...
-- ISLR
        ...
        FOR
            ...
            FROM
                dbo.saCobroDocReng R1
                INNER JOIN dbo.saDocumentoVenta DC ON DC.co_tipo_doc = R1.co_tipo_doc
                                                      AND DC.nro_doc = R1.nro_doc
                LEFT JOIN dbo.saCobroRentenReng R2 ON R1.rowguid = R2.rowguid_reng_cob
            WHERE
                R1.co_tipo_doc IN ( 'ISLR' )
            GROUP BY
                DC.saldo, DC.numcom, DC.feccom, DC.rowguid, DC.co_tipo_doc, DC.nro_doc, R1.mont_cob, DC.total_neto
            HAVING
                R1.mont_cob <> DC.total_neto
                OR R1.mont_cob <> ROUND(ISNULL(SUM(R2.monto_reten), 0), 2)
            ORDER BY
```
