# SP: pExportarActualizarRegistrosConfigProveedor
**Tipo**: Procedimiento
**Módulo**: Clientes
...
```sql
/**************************************************************************
*NOMBRE			:	pExportarActualizarRegistrosConfigProveedor
*DESCRIPCIÓN	:	Actualiza la configuracion del proceso ConfigProveedor
*AUTOR			:	SOFTECH SISTEMAS
***************************************************************************/
CREATE PROCEDURE [dbo].[pExportarActualizarRegistrosConfigProveedor]
    (
      @sCo_Config		CHAR(6) ,
...
AS 
    BEGIN
		...
		SET @sSqlValidador = N'	SELECT @tsValidadorOUT = validador FROM  [' + @sempresa + '].[dbo].[saConfigProveedor] WHERE co_config = @sCo_Config'  
		...
		SET @sSql = N'EXEC [' + @sempresa + '].[dbo].[pActualizarConfigProveedor]
																		@sCo_Config,
																		@sCo_ConfigOri,
																		@sDes_Config,
																		@sCo_Usuario,
