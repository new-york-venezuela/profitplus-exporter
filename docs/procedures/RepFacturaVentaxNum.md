...
-- Lastdate Update: <2018-02-01> 
-- Description:	<Reporte de Factura de Ventas por Número>
-- =============================================
CREATE PROCEDURE [dbo].[RepFacturaVentaxNum] 
...
-------------------------------
        SELECT
            @cAnulado AS Filtro_anulado, 'FACT' AS tipo_rep, CLI.tip_cli, CLI.cli_des, CLI.co_zon,
            ...
            FV.porc_desc_glob, FV.monto_desc_glob, FV.porc_reca, FV.monto_reca, 
                    FV.total_bruto, 
                    cast (FV.monto_imp / ( CASE WHEN @cCo_Moneda IS NULL THEN 1 ELSE FV.tasa END ) as decimal(18,2)) AS monto_imp,
            FV.monto_imp2, FV.monto_imp3, FV.otros1, FV.otros2, FV.otros3,
            cast (FV.total_neto / ( CASE WHEN @cCo_Moneda IS NULL THEN 1 ELSE FV.tasa END ) as decimal(18,2)) AS total_neto, 
                    cast (FV.total_neto / ( CASE WHEN @cCo_Moneda IS NULL THEN 1 ELSE FV.tasa END ) as decimal(18,2)) AS total_neto, 
                    cast
```
