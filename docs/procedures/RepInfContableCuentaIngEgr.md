...
-- Description:	<Información Contable de Cuenta Ingreso/Egreso>
-- =============================================
CREATE PROCEDURE [dbo].[RepInfContableCuentaIngEgr]
...
BEGIN
	SET NOCOUNT ON;
   Select A.co_cta_ingr_egr, A.num_carp, A.Descripcion, case when ltrim(A.monto) = '' then 0 else cast(A.monto as decimal(18,5)) end as monto,
   ...
   Select	A.co_cta_ingr_egr, '01' as num_carp,
	...
	from	saCuentaIngEgr A
		outer apply A.dis_cen.nodes('InformacionContable/Carpeta01/DistribucionCentros/CentroCosto') as C(N)
	Where 
		((@sCo_Cta_d IS NULL OR A.co_cta_ingr_egr >= @sCo_Cta_d) AND (@sCo_Cta_h IS NULL OR A.co_cta_ingr_egr <= @sCo_Cta_h))
...
UNION ALL
	...
	select	A.co_cta_ingr_egr, '02' as num_carp,
		...
		A.dis_cen.query( '/InformacionContable/C
```
