# SP: pInsertarRenglonesFacturaCompra
**Tipo**: Insertar
**Módulo**: Compras
...
## Código (excerpt)
```sql
CREATE PROCEDURE [dbo].[pInsertarRenglonesFacturaCompra]
    (
      @iReng_Num INT ,
      @sDoc_Num CHAR(20) ,
      @sCo_Art CHAR(30) ,
...
AS 
    BEGIN
		...
		IF(@sCredito_fiscal = 'NO Deducible (Art. 33)')
			SET @sAux = '0'
		ELSE IF (@sCredito_fiscal = 'Totalmente Deducible (Art. 34)')
			SET @sAux = '1'
		ELSE IF (@sCredito_fiscal = 'Sujeto a Prorrateo (Art. 34)')
			SET @sAux = '2'
		ELSE
			SET @sAux = '3'

		/*La cantidad de pendiente
```
