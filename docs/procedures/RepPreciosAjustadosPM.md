...
-- Description:	<Reporte de Precios Ajustados Proceso Manual>
-- =============================================
CREATE PROCEDURE [dbo].[RepPreciosAjustadosPM]
...
FROM saPista P INNER JOIN saAjPrecioCostoM AP ON P.rowguidOri = AP.rowguid
				INNER JOIN SaArticulo A ON (SUBSTRING(P.campos, 1, CHARINDEX(',', campos) - 1)) = A.co_art
WHERE P.tablaori='pActualizarPrecioCostoManual' AND AP.procesado = 1 
...
INTO @CADENA, @DESCRIP, @TIPAJU
	...
	BEGIN
		...
		@CADENA2 = SUBSTRING(@CADENA, CHARINDEX(',', @CADENA) +1, LEN(@CADENA)), -- co_alma

		@CADENA3 = SUBSTRING(@CADENA2, 1, CHARINDEX(',', @CADENA2) -1), -- Carlos
		@CADENA4 = SUBSTRING(@CADENA2, CHARINDEX(',', @CADENA2) +1, LEN(@CADENA2)), -- María,Josefina,Pedro,Jorge

		@CADENA5 = SUBSTRING(@CADENA4, 1, CHARINDEX(',', @CADENA4) -1), -- María
		@CADENA6 = SUBSTRING(@CADENA4, CHARINDEX(',', @CADENA4) +1, LEN(@CADENA4)), -- Josefina,Pedro,Jorge

		@CADENA7 = SUBSTRING(@CADENA6, 1, CHARINDEX(',', @CADENA6) -1), -- Jose
```
