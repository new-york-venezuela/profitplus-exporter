# SP: RepFacturaVentaconDevolucion
**Tipo**: Reporte
**Módulo**: Ventas
...
-- Description:	<Facturas de Ventas con sus Devoluciones>
-- =============================================
CREATE PROCEDURE [RepFacturaVentaconDevolucion]
    ...
    BEGIN
        ...
        FROM
            ( SELECT
				...
				dbo.fechasimple(P.fec_venc) as fec_vec, 
				P.co_mone, P.tasa,
                P.total_neto, P.co_cli, C.cli_des, 
				'Devolución' AS tip, N.total_neto AS neto, 
				dbo.fechasimple(N.fec_emis) AS fecha,
                NV.reng_neto, N.total_bruto AS bruto, 
				N.tasa AS tasar, N.co_mone AS mone, 
				P.co_sucu_in,
                'Factura' AS tipox
              FROM
                saFacturaVenta AS P
                INNER JOIN saFacturaVentaReng AS CR ON P.doc_num = CR.doc_num
                                                       AND P.anulado = 0
                INNER JOIN ( SELECT
