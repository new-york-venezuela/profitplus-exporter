# SP: pValidarPlantillaVentaCalculos
**Tipo**: Validar
**Módulo**: General
...
## Código (excerpt)
```sql
CREATE PROCEDURE [dbo].[pValidarPlantillaVentaCalculos]
    (
...
AS 
	...
	-- Total Bruto (Versus renglones)
        ...
        FOR
            ...
            FROM
                saPlantillaVenta E
                INNER JOIN saPlantillaVentaReng R ON e.doc_num = R.doc_num
            GROUP BY
        ...
        WHILE @@FETCH_STATUS = 0 
            BEGIN
                SET @PistaMensaje = 'La plantilla de venta nro. "' + RTRIM(@pDoc_Num)
                    + '" tiene como valor total bruto "' + LTRIM(RTRIM(STR(@pValorOld, 18, 2))) + '" y el correcto es "'
                    + LTRIM(RTRIM(STR(@pValorNew, 18, 2
```
