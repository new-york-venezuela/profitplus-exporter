# SP: pExportarActualizarRegistrosConfigOrdenCompra
**Tipo**: Procedimiento
**Módulo**: Compras
...
```sql
/**************************************************************************
*NOMBRE			:	pExportarActualizarRegistrosConfigOrdenCompra
*DESCRIPCIÓN	:	Actualiza la configuracion del proceso ConfigOrdenCompra
*AUTOR			:	SOFTECH SISTEMAS
***************************************************************************/
CREATE PROCEDURE [dbo].[pExportarActualizarRegistrosConfigOrdenCompra]
    (
      @sCo_Config		CHAR(6) ,
...
AS 
    BEGIN
		...
		SET @sSqlValidador = N'	SELECT @tsValidadorOUT = validador FROM  [' + @sempresa + '].[dbo].[saConfigOrdenCompra] WHERE co_config = @sCo_Config'  
		...
		SET @sSql = N'EXEC [' + @sempresa + '].[dbo].[pActualizarConfigOrdenCompra]
																		@sCo_Config,
																		@sCo_ConfigOri,
																		@sDes_Config,
																		@sCo_Usuario,
