...
-- Description:	<Información Contable de Segmento>
-- =============================================
CREATE PROCEDURE [dbo].[RepInfContableSegmento]
...
BEGIN
	SET NOCOUNT ON;
   Select A.co_seg, A.num_carp, A.Descripcion, case when ltrim(A.monto) = '' then 0 else cast(A.monto as decimal(18,5)) end as monto,
   ...
   Select	A.co_seg, '01' as num_carp,
	...
	Where 
		((@sCo_Seg_d IS NULL OR A.co_seg >= @sCo_Seg_d)		AND ( @sCo_Seg_h IS NULL OR A.co_seg <= @sCo_Seg_h))
...
UNION ALL
	...
	select	A.co_seg, '02' as num_carp,
