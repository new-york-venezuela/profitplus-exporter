# SP: pValidarDocumentoVentaGenAutomatica
**Tipo**: Validar
**Módulo**: Ventas
...
## Código (excerpt)
```sql
CREATE PROCEDURE [dbo].[pValidarDocumentoVentaGenAutomatica]
    (
                                                    ...
                                                    END + CASE WHEN DC.tasa <> F.tasa THEN ' Tasa,'
                                                          ...
                                                          END + CASE WHEN DC.N_Control <> F.N_Control THEN ' N_Control,'
                                                                ...
                                                                END + CASE WHEN DC.anulado <> F.anulado THEN ' Anulado,'
                                                                           ELSE ''
                                                                      END
                + CASE WHEN DC.Total_Bruto <> F.Total_Bruto THEN ' Total_Bruto,'
                       ELSE ''
                  END + CASE WHEN DC.Monto_Reca <> F.Monto_Reca THEN ' Monto_Reca,'
                             ELSE ''
                        END + CASE WHEN DC.Monto_Desc_Glob <> F.Monto_Desc_Glob THEN ' Monto_Desc_Glob,'
                                   ELSE ''
```
