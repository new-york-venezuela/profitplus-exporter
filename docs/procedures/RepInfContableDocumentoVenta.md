# SP: RepInfContableDocumentoVenta
**Tipo**: Reporte
**Módulo**: Clientes
...
-- Description:	<Información Contable Documento de Venta>
-- =============================================
CREATE PROCEDURE [dbo].[RepInfContableDocumentoVenta]
	@sNumero_d CHAR(20) = NULL,
	@sNumero_h CHAR(20) = NULL,
	@dFecha_d SMALLDATETIME = NULL,
    @dFecha_h SMALLDATETIME = NULL,
	@sTip_Doc CHAR(6) = NULL,	
...
BEGIN
  ...
  Select A.nro_doc, DOC.co_tipo_doc, DOC.fec_emis, DOC.co_cli, (select cli_des from saCliente where co_cli = DOC.co_cli) as cli_des, DOC.co_ven, 
  DOC.feccom, DOC.numcom, A.num_carp, A.Descripcion, case when ltrim(A.monto) = '' then 0 else cast(A.monto as decimal(18,5)) end as monto, 
  A.co_cue, A.co_gas, A.co_cen, A.des_cen, case when ltrim(A.porc) = '' then 0 else cast(A.porc as decimal(18,5)) end as porc,   
  CUE.des_cue, 
  GAS.des_gas
 From (

 Select	A.nro_doc, A.feccom, '01' as num_carp,
		...
		(select des_cen from scCentro where co_cen=C.N.value('(Codigo)[1]', 'varchar(6)')) a
```
