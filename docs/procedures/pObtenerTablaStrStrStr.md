...
***************************************************************************************************************/
CREATE PROCEDURE [pObtenerTablaStrStrStr]
    (
...
AS 
    BEGIN	
        ...
        SET @sParametro = N'@sCampoClave11 varchar(20), @sCampoClave22 varchar(20), @sCampoClave33 varchar(20)' 
        ...
        IF @iTipo = 1 
			...
			WHERE (((' + @sColumna1 + ' < @sCampoClave11) OR 
				  ...
				  ((' + @sColumna1 + ' = @sCampoClave11) AND (' + @sColumna2 + ' = @sCampoClave22) AND ('
                    + @sColumna3 + ' < @sCampoClave33))))and (' + @sColumna1 + ' = @sCampoClave11)' + 'ORDER BY '
                    + @sColumna1 + ' DESC,' + @sColumna2 + ' DESC,' + @sColumna3 + ' DESC  '			
            END	
