# SP: pValidarOrdenCompraCalculos
**Tipo**: Validar
**Módulo**: Compras
...
## Código (excerpt)
```sql
CREATE PROCEDURE [dbo].[pValidarOrdenCompraCalculos]
    (
...
AS 
	...
	-- Total Bruto (Versus renglones)
        ...
        FOR
            ...
            FROM
                saOrdenCompra E
                INNER JOIN saOrdenCompraReng R ON e.doc_num = R.doc_num
            GROUP BY
        ...
        WHILE @@FETCH_STATUS = 0 
            BEGIN
                SET @PistaMensaje = 'La orden de compra nro. "' + RTRIM(@pDoc_Num) + '" tiene como valor total bruto "'
