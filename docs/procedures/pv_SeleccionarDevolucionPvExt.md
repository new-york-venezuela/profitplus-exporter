# SP: pv_SeleccionarDevolucionPvExt
**Tipo**: Punto de Venta
**Módulo**: Tesorería
...
```sql
/*********************************************************************
*NOMBRE			:	[pv_SeleccionarDevolucionPvExt]
*DESCRIPCIÓN	:	OBTIENE LA INFORMACION ADICIONAL DE LA DEVOLUCION DE CLIENTE
					PARA PUNTO DE VENTA DESDE EL SISTEMA ADM 8.0
*AUTOR			:	SOFTECH SISTEMAS
*********************************************************************/ 
CREATE PROCEDURE [dbo].[pv_SeleccionarDevolucionPvExt]
( 
	@growguid_doc_num UNIQUEIDENTIFIER
)
AS 
    BEGIN
        SELECT p.rowguid_doc_num, S.num_turno, 
		S.co_turno,T.des_turno,
		S.cod_caja, T2.descrip,
		S.user_caj
			FROM pvDevolucionClienteExt P
