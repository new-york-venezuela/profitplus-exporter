# SP: pSeleccionarContabilizacionDevolucionClienteRenglon
**Tipo**: Seleccionar
**Módulo**: Inventario
...
## Código (excerpt)
```sql
CREATE PROCEDURE [dbo].[pSeleccionarContabilizacionDevolucionClienteRenglon]
    (
...
AS 
    BEGIN
       ...
       SELECT     FV.doc_num AS Co_Doc_Padre, FVR.reng_num AS Co_Doc, CASE WHEN FV.fec_emis > FV.fec_reg THEN FV.fec_emis ELSE FV.fec_reg END as fec_Emis, MO.relacion AS mone_relacion, CL.co_cli AS Co_Auxiliar, CL.cli_des AS Descrip_Auxiliar, 
                      ...
                      FVR.lote_asignado, FVR.dis_cen AS dis_cen_saDevolucionClienteRenglon, FVR.co_us_in, FVR.co_sucu_in, FVR.fe_us_in, FVR.co_us_mo, FVR.co_sucu_mo, 
		...
		FROM         dbo.saDevolucionClienteReng AS FVR INNER JOIN
                      dbo.saDevolucionCliente AS FV ON FVR.doc_num = FV.doc_num INNER JOIN
        ...
        WHERE            --(DATEDIFF(DAY, FV.fec_emis , @sdFechaDesde) <= 0 AND DATEDIFF(DAY, FV.fec_emis , @sdFechHasta) >=
```
