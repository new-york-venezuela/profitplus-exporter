...
-- Description:	<Reporte de Notas de Despacho de Venta por Fecha>
-- =============================================
CREATE PROCEDURE [dbo].[RepNotaDespachoVentaxFecha] 
...
-------------------------------
        SELECT
            @cAnulado AS Filtro_anulado, 'NOTA_DES' AS tipo_rep, @cImprSubTotal AS SubTotales, CLI.tip_cli, CLI.cli_des,
            CLI.co_zon, CLI.co_seg,
		/*Campos saFacturaVenta*/ FV.doc_num, FV.descrip, FV.co_cli, FV.co_tran, FV.co_mone, FV.co_ven, FV.co_cond,
			...
			FV.total_neto / ( CASE WHEN @cCo_Moneda IS NULL THEN 1
                                   ELSE FV.tasa
                              END ) AS total_neto, 
			FV.saldo / ( CASE WHEN @cCo_Moneda IS NULL THEN
```
