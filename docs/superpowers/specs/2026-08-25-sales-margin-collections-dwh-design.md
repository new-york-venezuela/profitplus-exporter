...
- **Inventory reality check — affects COGS join design**: per `saAlmacen` documentation, this installation is a **make-to-order bakery**. Finished-goods sales (166 active articles, `tipo='V'`) post against warehouse `000015` ("OFICINA"), which **never carries positive stock** in `saStockAlmacen`. Only 2 of 52 warehouses (`13`, `14`) carry real stock, and those are raw-material/supply warehouses, not finished goods. `saCostoHistoricoSalida` (the cost-consumption log intended to drive COGS) has only ~4,608 rows total across the system's lifetime.
  - **Open verification task (Phase 1 of implementation, not blocking this design)**: confirm via live query whether `saCostoHistoricoSalida` actually contains `tipo_doc='FACT'` rows for finished-goods sales lines, or whether it's populated almost exclusively by raw-material movements (`AJUS`, `TRAS`, `COMP`) against warehouses `13`/`14`. If finished-goods sales lines have **no** cost-history rows, Fact_Sales cannot compute a true FIFO COGS per sale and must fall back to `saArticulo.tipo_cos`-driven costing (last cost / average cost from `saCostoHistoricoEntrada`, joined by article rather than by transaction) — this is a materially different (and less precise) margin methodology and needs to be flagged to the business stakeholder before the Gross Margin Waterfall dashboard is presented as authoritative.
...
| `UnitCost` | `saCostoHistoricoSalida.costo_pro` joined via `saArticulo.rowguid` + `doc_orig`, **or** fallback per §2 | Flagged with `CostSourceFlag` ('HISTORY' vs 'FALLBACK_AVG_COST') so margin dashboards can show data-quality confidence |
| `COGSAmount` | `UnitCost × QuantitySold` | — |
| `GrossProfitAmount` | `NetAmount - COGSAmount` | — |
...
1. **COGS join fidelity** (§2) — must verify live whether `saCostoHistoricoSalida` covers finished-goods `FACT` sales lines before the Gross Margin dashboard can be trusted at line grain. This is the single highest-risk item in the whole design.
