...
-- Date		  : 16/07/2010
-- =============================================
CREATE TRIGGER [dbo].[TrigEstado_saArtCompuestoGen]
   ON  [dbo].[saArtCompuestoGen]  FOR INSERT,UPDATE
AS 
...
BEGIN
	...
	Select rowguid,gene_art from inserted
	...
	BEGIN
		...
		Select @EstadoOld = gene_art from deleted where deleted.rowguid = @Id 

		If (@EstadoOld is null or @EstadoOld <> @EstadoNew)
		Begin
			Insert into saHistoricoEstado (	doc_orig, tipo_doc, Estado, fecha) values
				(@Id, 'saArtCompuestoGen',  cast(@EstadoNew as char(4)), getdate())
		End
